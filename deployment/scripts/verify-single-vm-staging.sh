...
contract_filter=$script_dir/../rehearsals/single-vm-staging-contract.jq
[ -f "$contract_filter" ] || {
  echo "Single-VM staging contract filter not found: $contract_filter" >&2
  exit 1
}
...
echo "Provider-neutral single-VM staging Compose contract validated."
