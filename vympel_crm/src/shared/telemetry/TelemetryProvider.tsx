...
import {createTelemetryDeduper, reportTelemetry, type TelemetryEvent} from "@/shared/telemetry/telemetry";

const isFirstReport = createTelemetryDeduper();

function context(): Pick<TelemetryEvent, "route" | "locale" | "deviceClass"> {
  const width = window.innerWidth;
  const savedLocale = window.localStorage.getItem("vympel_crm_locale");
  return {
    route: window.location.pathname,
    locale: savedLocale ?? document.documentElement.lang,
...
export function TelemetryProvider({children}: {children: ReactNode}) {
  useReportWebVitals((metric) => {
    ...
    reportTelemetry({kind: "web_vital", name: metric.name, value: metric.value, rating: metric.rating, ...context()});
  });
