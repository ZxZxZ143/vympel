...
type ReporterConfig = { enabled: boolean; sampleRate: number; random?: () => number };
...
export function sanitizeTelemetryEvent(input: unknown): TelemetryEvent | null {
  ...
  if (!( ["runtime_error", "react_boundary", "api_error", "web_vital"] as const).includes(source.kind as TelemetryKind)) return null;
  return {
    ...
    status: typeof source.status === "number" && source.status >= 100 && source.status <= 599 ? Math.trunc(source.status) : undefined,
    ...
    deviceClass: source.deviceClass === "mobile" || source.deviceClass === "tablet" || source.deviceClass === "desktop"
      ? source.deviceClass
      : undefined,
    value: typeof source.value === "number" && Number.isFinite(source.value)
...
export function shouldSample(sampleRate: number, random = Math.random) {
