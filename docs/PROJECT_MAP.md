...
* Frontend tests: Public Vitest covers recommendation behavior, one-request/in-flight-deduplicated batch summary, batch snapshot/unavailable merging, 429 parsing, CMS revalidation signature/payload/target mapping, and the exact six canonical brand routes. On the audited 2026-08-07 storefront commit it runs 19 files / 62 tests. CRM Vitest covers auth/session behavior, explicit CMS partial-success feedback, and canonical brand-country derivation. Both apps require lint and production builds; public additionally runs typecheck and the isolated production status/content matrix.
...
2026-08-10 - Implemented the 28-item storefront accessibility/SEO ledger: fail-closed indexing, localized metadata/canonical/URL policy, SSR catalog/product foundations, honest states, focus-managed disclosures/dialogs/forms/carousels, real structured data/social metadata, locale mapping, and performance regression gates.
