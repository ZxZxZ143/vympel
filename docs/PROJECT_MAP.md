...
| CRM product detail | `GET /api/crm/products/{id}?lang=` | Bearer access token | `ProductResponse` including category, brand, collection, images, localized descriptions/names, stock/marketplace links, and profile-specific optional details. Wristwatch details include legacy fields plus dial type/marking, power source, normalized water resistance, shared strap/dial colors, package contents, and a localized feature list. | CRM edit form |
...
2026-09-04 - Completed optional watch characteristics and normalized multi-feature persistence across DB/backend/CRM/bulk/storefront, added authenticated creatable-reference APIs and reusable inline-create UX, extended live Kaspi mappings, verified the guarded Liquibase upgrade and full local browser/API round-trip, and updated the deployment migration boundary.
