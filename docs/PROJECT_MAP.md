...
* Key libraries: Public storefront uses `next-intl` 4.13.2, `react-hook-form`, `zod`, `class-variance-authority`, `clsx`, `tailwind-merge`; both Next.js apps use `react-markdown` 10.1.0 with `remark-gfm`, `remark-breaks`, `rehype-raw`, and `rehype-sanitize` for safe product-description Markdown, while CRM also uses local API/i18n/UI helpers.
...
* Key libraries: Liquibase, JJWT 0.12.6, Spring Data Redis, jsoup 1.23.2, MapStruct 1.5.5.Final, Lombok, springdoc OpenAPI, AWS SDK S3, Logback rolling-file appenders, Testcontainers.
...
|-- infrastructure/
|   |-- compose/                       # prebuilt-image provider, reusable single-VM, and Oracle staging/production stacks
|   |-- env/                           # placeholder-only deployment environment templates, including GCP and Oracle staging
|   |-- observability/                 # provider-neutral Prometheus/rule/signal examples
|   |-- reverse-proxy/                 # configurable TLS, provider-neutral/Oracle HTTP-first, and internal media-gateway Nginx
...
|   |-- audits/
|   |   |-- FRONTEND_ACCESSIBILITY_SEO_AUDIT.md # exact-commit storefront WCAG/SEO/crawl/performance evidence and launch checklist
|   |   `-- CODEX_FRONTEND_ACCESSIBILITY_SEO_IMPLEMENTATION_PROMPT.md # ordered implementation handoff for the audit ledger
...
| Root layout | `vympel_front/src/app/[locale]/layout.tsx`, `src/components/Providers/NProgressProvider.tsx`, `navigationProgressController.ts`, `navigationProgressPolicy.ts` | Loads fonts, locale messages, header/footer, the Suspense-wrapped mobile bottom navigation, the project-restyled shadcn/Sonner toaster, shadcn Tooltip provider, the global customer-request dialog provider, and the public-storefront-only NProgress lifecycle. Navigation progress finishes on committed path/query changes, history navigation, errors, locale-layout teardown, or a finite failsafe; reduced-motion users receive an immediate, non-trickling transition. | `RootLayout`, `NProgressProvider`, `startNavigationProgress`, `finishNavigationProgress`, `shouldStartNavigationProgress`, `generateStaticParams`, `metadata` |
| SEO and discovery | `vympel_front/site-indexing.mjs`, `src/lib/siteIndexing.ts`, `src/lib/seo.ts`, `src/lib/seoContent.ts`, `src/lib/structuredData.ts`, `src/lib/catalogStructuredData.ts`, `src/lib/catalogUrl.ts`, `src/app/sitemap.ts`, `src/app/robots.ts`, localized route metadata | Fails closed unless server-only `SITE_INDEXING_ENABLED=true` and an origin-only HTTPS `NEXT_PUBLIC_SITE_URL` are both present. Disabled builds emit global/header/meta `noindex, nofollow`, no canonical/alternates/social metadata, an empty sitemap, and non-advertising robots. Enabled Home/About/Brands/Brand/Catalog/Category/Product/Delivery/Payment/Guarantee routes own truthful route-specific RU/KK/EN titles/descriptions plus matching canonical/hreflang/OG/Twitter. Product metadata reuses the memoized route loader, deduplicates brand/model identity, removes HTML/Markdown/extra whitespace from real localized product copy, and shortens at a word boundary. Clean catalog URL policy and real Product/Breadcrumb JSON-LD remain unchanged; private, error, transient, search, filter, and non-default-sort states are not indexed. | `readSiteIndexingConfig`, `publicSeoMetadata`, `staticSeoContent`, `categorySeoContent`, `brandSeoContent`, `productSeoContent`, `plainSeoText`, `productStructuredData`, `breadcrumbStructuredData`, `classifyCatalogUrl`, `sitemap`, `robots` |
| i18n routing | `vympel_front/src/i18n/*`, `vympel_front/src/hooks/useProgressRouter.ts`, `vympel_front/src/proxy.ts` | Supports `ru`, `kz`, `en`; redirects unsupported locale-like prefixes; wraps localized links and imperative App Router navigation with the storefront progress lifecycle. The shared `Link` starts progress only after consumer click handlers have had a chance to cancel and ignores external, new-tab, download, modified, non-left, hash-only, and current-URL clicks. | `LocaleEnum`, `routing`, `Link`, `useProgressRouter`, `useProgressIntlRouter` |
...
| Public CMS helpers | `vympel_front/src/utils/cmsContent.ts`, `src/components/ui/shared/CmsResponsiveImage/index.tsx` | Safely selects CMS blocks by key/type, resolves localized desktop/mobile media with static fallback, appends stable media/block version query params so updated CMS images do not reuse stale browser/CDN entries, replaces failed dynamic image URLs with the known static fallback, extracts text fields, and resolves CMS links to safe external URLs, internal paths, or semantic route helpers for category, brand, product, and catalog-filter targets. Known Kaspi/Wildberries CMS URLs are canonicalized through `MARKETPLACE_LINKS`, marketplace links always open in a new tab, and Ozon destinations resolve to no public link. Mobile image fallback order is current-locale mobile, current-locale desktop, default/RU mobile, default/RU desktop, then static fallback. | `findCmsBlock`, `findCmsBlocksByType`, `cmsImageSources`, `cmsText`, `cmsTextList`, `cmsLink`, `CmsResponsiveImage` |
| Home screen | `vympel_front/src/screens/HomePage/index.tsx`, `src/components/HomePage/bannerCarousel/*`, `src/components/HomePage/ProductCarouselSection/index.tsx`, `src/components/MarketPlaces/index.tsx` | Composes navigation, tall responsive hero/banner carousel, benefits, new arrivals, brands, categories, an accessories rail immediately after categories, mobile-redesigned philosophy/story block, and marketplaces. The multi-slide hero is the storefront's only autoplay carousel: it starts automatically when reduced motion is not requested, pauses across whole-carousel mouse hover and keyboard-origin descendant focus, resumes after leave/blur and drag/touch completion, and has no visual Play/Pause control. A shared lifecycle owns Embla reInit, visibility restoration, listener/frame cleanup, pointer-versus-keyboard focus semantics, and ignores the shared Carousel's programmatic root-focus fallback after a dragged focused slide becomes hidden. Shared carousel controls have an explicit stacking layer so slide images cannot intercept a visible arrow. New arrivals and accessories share the server-rendered `ProductCarouselSection`, which loads real category products through the existing by-code API, preserves backend availability-first/newest ordering, maps locale-aware product/catalog routes, hides genuinely empty sections, and delegates card/swipe/arrow/banner behavior to `GoodsCarouselWithImage`. The accessories rail uses `/accessories-banner.png`; the new-goods banner remains CMS-first with its existing static fallback. The marketplace section contains only Kaspi and Wildberries: one full-width column on narrow screens and two equal-height columns from `md`; its always-visible mobile CTA uses a 60% black overlay so white text meets contrast requirements. Home product rails keep the controlled desktop banner layer, banner hidden on tablet/mobile when it becomes visual noise, and non-overlapping arrows. | default async `HomePage`, `ProductCarouselSection` |
...
| About page | `vympel_front/src/app/[locale]/about/page.tsx`, `src/screens/AboutPage/index.tsx`, `src/components/AboutPage/InstagramSlider/index.tsx`, `src/utils/aboutInstagramPosts.ts`, `src/utils/instagramPostUrl.ts` | Renders the localized About Us page from the Figma About frame with standard navigation/footer, CMS-first full-width banner, CMS-first intro/cooperation content, four numbered company cards, and a manual-only Embla Instagram rail. Published `INSTAGRAM_POST` blocks are sorted by CMS order and render lazy image links to canonical Instagram posts/reels; when none are usable, exactly four non-linked static cards (`/insta-1.webp`, `/insta-2.png`, `/insta-3.webp`, `/insta-4.webp`) render without mixing or duplication. Static localized messages and banner/contact assets remain fallbacks. | default async `AboutPage`, `AboutInstagramSlider`, `resolveAboutInstagramPosts`, `canonicalInstagramPostUrl` |
| Brand page config | `vympel_front/src/config/brandRoutes.ts`, `vympel_front/src/config/brandPages.ts` | Central source for the six supported public brand slugs, display names, and canonical database codes, including `pierre-ricaud`; explicit RU/KZ/EN brand copy with current-locale-to-RU fallback; and banner asset mapping/fallbacks. Runtime legacy brand aliases are intentionally not accepted. | `PUBLIC_BRANDS`, `findPublicBrandBySlug`, `getBrandPageData` |
| Public route helpers | `vympel_front/src/config/routes.ts`, `src/config/socialLinks.ts`, `src/config/brandRoutes.ts`, `src/app/[locale]/brands/[brandSlug]/page.tsx` | Central source for public internal hrefs, semantic catalog/category/filter presets, optional validated social links, marketplace links, and product/brand/cart/favorites/about/info-page route builders. Clean categories use `/catalog/{categoryCode}`; legacy `categoryCode` query URLs permanently redirect while preserving normalized facets. Search/filter/non-default sort remain query parameters and reset pagination. `MARKETPLACE_LINKS` contains only canonical Kaspi/Wildberries destinations. Instagram renders as a link only when `NEXT_PUBLIC_INSTAGRAM_URL` is a validated non-root Instagram profile URL; absent/unapproved configuration remains non-interactive. | `routes`, `catalogLinks`, `getSocialLinks`, `PUBLIC_CATEGORY_CODES`, `SEEDED_FILTER_VALUES`, `CONTACT_LINKS`, `MARKETPLACE_LINKS`, `resolveMarketplaceHref`, `PUBLIC_BRANDS` |
...
| Catalog screen | `vympel_front/src/app/[locale]/catalog/page.tsx`, `catalog/[...slug]/page.tsx`, `src/lib/catalogPageData.ts`, `src/lib/catalogQuery.ts`, `src/screens/CatalogPage/index.tsx`, `components/CatalogPage/*` | Server-loads the first requested catalog page so initial HTML contains truthful product headings/links and real anchor pagination; the client is seeded from that payload and does not duplicate the initial fetch. Confirmed absence/out-of-range returns 404, transient API failure renders a retryable non-404 state, and honest empty results stay empty. CMS hero/contact, toolbar, overlays, responsive three-column grid, category/filter/sort controls, and later client URL updates retain the existing visual design. | `CatalogPage`, `Catalog`, `loadCatalogPageData`, `catalogRequestParams`, `CatalogOverlayProvider`, `CatalogMobileSheet`, `CategorySelector`, `CatalogFilters`, `Sort` |
...
| Product screen | `vympel_front/src/screens/ProductPage/index.tsx`, `components/ProductPage/*`, `components/ProductRating/RatingStars.tsx` | Resolves primary product/reviews/CMS content first and streams dedicated recommendations independently through `AsyncProductRecommendations` inside null-fallback server `Suspense`. `ProductRecommendations` renders the title/carousel only for non-empty items; timeout/429/error/true exhaustion produces no heading, message, retry control, blank wrapper, spacing, or main-content delay. `ProductSummary` renders `ProductVariantSelector` in the right information column directly after the title and before gender when a derived model group has at least two public variants. The compact semantic thumbnail list preserves the backend's product-ID-ascending order on every sibling route, links to each independent localized product, loads object-storage images directly with a truthful fallback, and uses neutral one-pixel rounded borders with exactly one token-backed black selected border. Keyboard focus replaces the border with one inset outline rather than adding a second ring or shadow. The rail remains horizontally scrollable without widening the mobile page. Product actions, price, stock, gallery, characteristics, favorites, cart, and analytics still belong to the selected product only. `ProductSummary` does not render direct Kaspi/Wildberries purchase anchors or an empty marketplace wrapper; the cart/favorite action row and unavailable-product request CTA follow price by the named 28px `--spacing-product-summary-actions-gap`. Wristwatch specs render optional localized dial type/marking, power source, water-resistance option, strap/dial colors, package contents, and comma-joined feature names while omitting every missing value and internal ID. | default async `ProductPage`, `ProductVariantSelector`, `AsyncProductRecommendations`, `ProductRecommendations`, `loadProductRecommendations`, `ProductInfoTabs`, `ProductSpecs`, `ProductReviews`, `RatingStars` |
...
| URL interaction hooks | `vympel_front/src/hooks/usePagination.tsx`, `useSort.ts`, `useProgressRouter.ts` | Keeps catalog page and sort state in query params; pagination can optionally scroll to a target element such as the catalog product grid after URL page changes. Progress-aware raw and localized router wrappers start the global storefront bar for imperative path/query mutations and clean synchronous routing failures. | `usePagination`, `useSort`, `useProgressRouter`, `useProgressIntlRouter` |
...
| Product info tabs and Markdown description | `vympel_front/src/components/ProductPage/ProductInfoTabs/index.tsx`, `ProductDescription/index.tsx`, `src/lib/markdownSanitization.ts`, `src/screens/ProductPage/index.tsx` | Client-side tabs for description, warranty, delivery, order/payment, and reviews, with the product description supplied as a server-rendered React slot so the Markdown parser is not shipped in the interactive tabs bundle. `ProductDescription` renders `content`/`shortText` through GFM plus ordinary-line-break support, parses raw HTML only so sanitized `<u>` is available, strips scripts/styles/event attributes/unsafe URL protocols, and applies restrained responsive `.product-markdown` typography. | `ProductInfoTabs`, `ProductDescription`, `markdownSanitizationSchema` |
...
| CRM API client | `vympel_crm/src/shared/api/client.ts`, `session.ts`, `types.ts`, `errors.ts` | Dedicated credentialed no-store CRM fetch client with base URL config, sessionStorage access-token handling, HttpOnly refresh-cookie bootstrap, cross-tab Web Lock refresh serialization, single-flight 401 refresh plus one bounded retry, 403 session preservation, parsed 429 body/header cooldowns, and abort-aware request deadlines (15 seconds normally, 30 seconds for the remote Kaspi preview, 60 seconds for uploads). Login preserves email, localizes invalid credentials, disables resubmission during the localized RU/KZ/EN cooldown, and never retries credentials automatically. | `crmApi`, `CrmApiError`, `getCrmErrorMessage`, CRM types |
...
| CRM localization | `vympel_crm/src/shared/i18n/messages.ts`, `useI18n.ts` | Localized RU/KZ/EN CRM strings and locale state for all CRM UI labels, validation messages, mutation feedback, CMS editor labels/states, product photo upload/order/main/delete states, nav, tables, states, and every backend audit event currently emitted by CMS, review, request, user, and product workflows. | `messages`, `useI18n` |
| CRM UI primitives | `vympel_crm/src/shared/ui/*`, `src/app/globals.css` | Local `Text`, `Heading`, `Button`, semantic `ButtonLink`, `Field`, and `ConfirmDialog` primitives plus CRM-specific global design tokens inspired by the public storefront. Wide table wrappers are keyboard-focusable and show a focus outline, and danger text uses a WCAG-AA contrast-safe token. | `Text`, `Heading`, `Button`, `ButtonLink`, `Field`, `ConfirmDialog` |
| CRM product management, model groups, Kaspi preview, creatable references, and Markdown editor | `vympel_crm/src/features/products/*`, `src/shared/markdown/*` | Pageable newest-first no-store product management plus category-first create/edit, inline collections, images, quick edits, and bulk creation. `ProductModelVariantStrip` shows the complete bounded model family inside each matching list row and edit form while every product remains a separate row, route, mutation target, price, stock, image set, SKU, and marketplace record. It renders the same product-ID-ascending order for every current sibling and mirrors the storefront's neutral rounded border, single black selected border, subtle hover, and inset keyboard-focus treatment. The backend enriches only the current page in one batch variant query, so search/status filtering and product pagination keep their ordinary row/count semantics while siblings on other pages remain represented in the strip. Wristwatch create/edit/bulk forms expose optional dial type/marking, power source, shared strap/dial colors, package contents, and a normalized multi-select feature set; profile changes clear stale detail state. `CreatableReferenceFields` composes reusable localized selects/multi-selects with a focus-trapped create dialog, synchronous double-submit guard, immediate option-cache merge, and automatic selection through RHF-owned form state. The create-only Kaspi action appears after category selection, opens an accessible localized dialog, validates a product URL, displays mapped/unmapped/unresolved preview results, and applies only present resolved values—including a trimmed model—into the existing RHF form; unresolved or absent values preserve manual entries, stale inline-collection state is cleared when the imported brand changes, and the ordinary create submission remains the only persistence path. The preview dialog is a viewport-bounded flex column: header/footer/close stay fixed, only its focusable named body scrolls, long marketplace text wraps without horizontal overflow, background scrolling is locked, and focus trap/Escape/close/restore behavior remains deterministic down to 320px and short-height viewports. Watch, interior-clock, and accessory technical characteristics remain optional and profile-scoped. The RU/EN/KZ product-description fields use the shared lightweight `MarkdownEditor` and the storefront sanitizer contract. | `ProductListView`, `ProductForm`, `ProductModelVariantStrip`, `CreatableReferenceFields`, `KaspiImportDialog`, `applyKaspiImportPreview`, `MarkdownEditor`, `MarkdownContent`, `BulkProductCreateView`, `brandCountryFor` |
...
| CRM CMS editor | `vympel_crm/src/app/cms/page.tsx`, `src/features/cms/CmsView.tsx`, `src/features/cms/cmsBlockSchemas.ts`, `src/features/cms/instagramPostUrl.ts`, `src/features/cms/cmsRefreshFeedback.ts` | Admin-only content-management surface for selecting pages, creating/editing/reordering/publishing blocks, and previewing every editable block. A centralized per-type schema controls whether text, image, link, button, alt, settings, or JSON fields render and are submitted. The About page additionally exposes a repeatable Instagram-post section with image upload/preview, exact post/reel URL, RU/KZ/EN alt text, order, status, publish/unpublish, and delete; client-side URL normalization provides immediate feedback while backend policy remains authoritative. Save/delete/reorder/publish/unpublish reports committed content separately from public-cache state. | `CmsView`, `cmsBlockSchemas`, `canonicalInstagramPostUrl`, `getCmsRefreshFeedback` |
...
| Product services | `services/product/*`, `db/repositories/product/ProductModelVariant*.java`, `dtos/product/ProductModelVariant*.java`, `services/catalog/SupportedBrandCountry.java`, `services/catalog/SupportedCatalogDomainService.java`, `services/marketplace/MarketplaceUrlPolicy.java`, `productName/*`, `productDescription/*`, `watchDetail/*`, `accessoryDetail/*`, `categoryProduct/*`, `mappers/product/ProductDescriptionMapper.java` | Product creation/read models, bulk orchestration, collision-safe SKU generation, localized names/descriptions, category links, category-profile validation, canonical marketplace URL enforcement, and one authoritative six-brand/five-country domain. `ProductModelVariantService` derives groups from brand, trimmed upper-case model, and compatible category profile; its bounded native projection returns localized names, status, and main-or-first image for all anchors on the current CRM page in one call. Candidate rows have one canonical product-ID-ascending order independent of which group member is the request anchor. Public groups include only ACTIVE products whose complete category path is active and reaches a root; CRM groups include every status. There is no variant-group table, so model edits move products immediately. The first generated SKU remains readable; a legitimate collision receives a bounded random suffix instead of blocking same-model variants. Watch details validate active type-scoped dictionary rows for optional dial type/marking, power source, water resistance, and shared COLOR rows for strap/dial colors; package contents is bounded text, and zero-to-many watch features replace the existing `watch_details_feature` relation without duplicate links. | `ProductService`, `ProductModelVariantService`, `ProductBulkCreationService`, `SKUService`, `SupportedBrandCountry`, `SupportedCatalogDomainService`, `MarketplaceUrlPolicy`, `WatchDetailServiceImpl`, `InteriorClockDetailServiceImpl`, `AccessoryDetailService` |
| Kaspi product preview importer | `services/marketplace/kaspi/*`, `dtos/product/KaspiProductImport*.java`, `exceptions/ProductImportException.java` | Fetches one allow-listed HTTPS Kaspi product page through a fair bounded concurrency gate with explicit redirect revalidation, DNS/private-network rejection, direct no-proxy HTTP, one total deadline, content-type/encoding checks, and a cancelling 2 MiB streaming cap. It parses JSON-LD first with constrained embedded/semantic DOM fallbacks, ignores aggregate rows, rejects ambiguous numeric/range/fraction/length input instead of guessing, and resolves fields through category-profile-specific exact/normalized/alias dictionaries. An exact `Модель`/`Model`/`Моделі` leaf is promoted to the normal trimmed product model only when unambiguous; conflicts remain unresolved with `MODEL_AMBIGUOUS`, and no suffix stripping or fuzzy grouping is attempted. Wristwatch mapping includes dial type/marking, power, shared colors, package contents, feature lists, and reliable live leaf labels such as `Функции: хронограф` or `Отображение даты: число`; explicit negative feature values are never selected. Unknown/ambiguous data stays unresolved/unmapped, conflicts are reported, and preview never calls reference or product persistence. | `KaspiUrlGuard`, `SecureKaspiPageFetcher`, `KaspiProductParser`, `KaspiCharacteristicMapper`, `KaspiProductImportService`, `KaspiProductImportResponse` |
| Migration boundary verification | `deployment/LiquibaseChangeBoundary.java`, `deployment/MigrationVerificationRunner.java`, `deployment/scripts/generate-published-release-manifest.sh` | Derives the packaged final Liquibase identity from the master changelog for finite migration startup and release-manifest generation. Runtime verification requires that identity, rejects any applied database change absent from the packaged graph, and permits a packaged `runOnChange` entry to be the latest execution without mistaking it for a future migration. | `LiquibaseChangeBoundary`, `MigrationVerificationRunner` |
...
| Catalog services | `services/catalog/*`, `db/repositories/product/CatalogFacetRepository.java`, `db/repositories/product/PublicProductSummaryRepository.java` | Resolves category profiles/inheritance and specifications; selects only paginated/quick-search IDs with deterministic in-stock-first sorting; hydrates all cards in one compact projection; and returns only the supported localized brand/country facets through one base plus at most one profile aggregation. Unsupported numeric brand/country query IDs are discarded safely instead of changing results or surfacing invalid options. Metrics record bounded duration/query counts, and 2-second query timeouts bound the native reads. | `ProductCatalogService`, `CatalogCategoryProfileService`, `SupportedCatalogDomainService`, `PublicProductQueryService`, `CatalogFacetRepository`, `PublicProductSummaryRepository` |
...
| CMS services | `services/cms/*`, `services/marketplace/MarketplaceUrlPolicy.java`, `services/social/InstagramPostUrlPolicy.java` | CMS page/block/media business rules, centralized `CmsBlockSchema` capability enforcement, RU/EN/KK translation persistence with field-level RU fallback, published-only reads, ordering/link validation, and all six desktop/mobile locale references. `INSTAGRAM_POST` is valid only on `about`, requires an image and `EXTERNAL_URL`, stores only canonical HTTPS `instagram.com`/`www.instagram.com` `/p/{id}` or `/reel/{id}` URLs with `NEW_TAB`, and rejects unsafe/malformed variants. Existing media lifecycle, collision-free order, audit, and durable page-key revalidation outbox behavior is reused unchanged for every Instagram create/update/delete/reorder/publish/unpublish mutation. | `CmsService`, `CmsServiceImpl`, `CmsBlockSchema`, `MarketplaceUrlPolicy`, `InstagramPostUrlPolicy`, `CmsMediaReferenceService`, `CmsMediaCleanupService`, `CmsRevalidationOutboxService`, `PublicCmsCacheInvalidationService` |
...
| Object storage | `s3/*`, `services/objectStorage/ObjectStorageService.java`, `services/objectStorage/ObjectStorageDeletionOutboxService.java`, `services/objectStorage/ObjectStorageDeletionJob.java`, `db/entity/storage/ObjectStorageDeletionJob.java` | S3-compatible upload/download/link generation with separate internal client and browser-facing public endpoints, validated storage properties, product/CMS image MIME+extension/count/size validation, UUID object-key generation, partial-upload cleanup, locked ordered product-media persistence, one-main-image selection, reorder/delete handling with next-main promotion, CMS object upload under the `cms/` storage path, and shared admin/CRM image storage. Database-owned media deletion queues object keys transactionally before rows cascade; a retrying outbox worker removes the objects after commit so database rollback and storage failure cannot silently orphan either side. Product responses expose safe image DTOs; card/search links resolve main image, first ordered image, then `null`. | `StorageProps`, `S3Config`, `ObjectStorageService`, `ObjectStorageDeletionOutboxService`, `ObjectStorageDeletionJob` |
...
| Audit persistence | `db/entity/audit/CrmActivity.java`, `db/repositories/audit/CrmActivityRepository.java`, `services/crm/CrmActivityService.java` | CRM audit/activity event storage with actor, event, entity, metadata JSON, IP, user-agent, and timestamp. Security- and integrity-sensitive mutations persist their audit event in the same transaction; retention cleanup is bounded and concurrency-safe. | `CrmActivity`, `CrmActivityRepository`, `CrmActivityService` |
...
Derived model-variant flow:

```text
Public product detail or bounded CRM product page
  -> ProductModelVariantService receives one or several anchor product IDs
  -> one native projection joins only exact brand + upper(trim(model)) candidates
  -> recursive category ancestry derives a compatible product profile and public visibility
  -> window functions keep one product-ID-ascending order for every anchor, exact total, and at most 24 returned variants
  -> ObjectStorageService converts the selected main-or-first media keys to public URLs
  -> Product/PublicProduct/CRM list DTO embeds modelVariantGroup
  -> storefront/CRM renders canonical links to independent product routes
```
...
Category selector open/select
  ...
  -> selected root/child writes /{locale}/catalog/{code} through routes.categorySelectionCatalog
  -> previous price/filter/page params are cleared while search/sort can remain
  -> Catalog and CatalogFilters receive categoryCode from the canonical path after reload
...
User focuses SmartSearch in Navigation, CatalogPage, or ProductPage
  ...
  -> Enter/search/show-all routes to /{locale}/catalog?search={query} through routes.searchCatalog, clearing category/filter/price state and default page by construction
```
...
Visible public link or CTA
  ...
  -> /{locale}/catalog/{categoryCode} for category links
  -> /{locale}/catalog/WATCH_WRIST?gender=1 for semantic male wristwatch links
  -> /{locale}/brands/{brandSlug} for public brand pages, or /{locale}/catalog?brand={backendFilterValue} after resolving backend brand filter metadata
```
...
CRM /products/new
  ...
  -> optional Kaspi import posts {url, categoryId} to /api/crm/products/import/kaspi
  -> backend validates/fetches/parses/maps a preview without persistence; CRM shows mapped, unmapped, unresolved, and warning groups
  -> Apply merges only present resolved preview values into RHF while preserving category and manual values that have no resolution
...
ADMIN opens CRM /cms
  ...
  -> about/INSTAGRAM_POST requires one image and a strict canonical Instagram /p/{id} or /reel/{id} HTTPS URL; the backend forces EXTERNAL_URL + NEW_TAB
  ...
  -> About maps usable published Instagram blocks in CMS order; any non-empty CMS set completely replaces the four static non-linked fallback cards
...
| Product detail | `GET /api/public/product/{lang}/{id}` | Path `lang`, `id` | `PublicProductResponse` with SKU, localized name, model, canonical price/stock, category/brand/collection, ordered images, localized description, profile details, marketplace/promotion/rating fields, and `modelVariantGroup`. The group contains deterministic product-ID-ascending ACTIVE/public siblings independent of the requested current member, exact `total`, `truncated`, and at most 24 lightweight entries with main-or-first image or null. | `PublicApiController.getProduct`, product gallery, `ProductVariantSelector`, and favorites/cart snapshot refresh |
...
| Product analytics tracking | `POST /api/public/analytics/products/events` | `{ productId, eventType, sessionId?, metadata? }`; product/event must be valid, session is capped to 100, metadata is capped to 10 entries with nonblank keys up to 50 and primitive/null values up to 200 characters | `{ tracked: true }` for a persisted event or `{ tracked: false }` for a recent source/event/product/session duplicate; invalid/oversized input and throttled requests do not persist | Product page view tracking, favorite toggle tracking, add-to-cart/remove-from-cart tracking, and canonical marketplace CTA clicks |
...
| Catalog products | `GET /api/public/product/catalog/{lang}` | Query `categoryCode?`, nonblank `search?`, actual `priceMin?`/`priceMax?` or aliases `minPrice?`/`maxPrice?`, pageable `page/size/sort`, and repeated selected filter keys such as `brand`, `country`, `mechanism`, `gender`, `caseMaterial`, `strapMaterial`, `glassType`, `stoneInlay`, `caseSize`, `interiorColor`. Accessories intentionally send only the simple `gender` filter from their toolbar toggles; stale full-filter and price params are omitted by the frontend. Empty arrays, null/undefined placeholders, blank strings, missing keys, metadata-only filter keys, removed country aliases (`brandCountry`, `manufacturerCountry`, `countryOfBrand`), and unsupported numeric brand/country IDs are ignored. Backend caps public page size to 60 and allow-lists product sort fields. | `Page<ProductShortResponse>` with approved-only rating summary, filtered by backend category profile and ordered by computed in-stock bucket before selected sort and pagination; text search matches globally and uses PostgreSQL trigram similarity plus `LIKE`; catalog cards may have `imageUrl: null` when product media is missing | Public catalog grid, global search, brand pages loading up to 15 newest same-brand products with `brand={id}`, and favorites similar-product fallback |
...
| Catalog filter metadata | `GET /api/public/product/filters/{lang}?categoryCode=` | Optional `categoryCode` | `CatalogFiltersResponse` with category context, `inheritsFiltersFrom`, localized filter labels, `source`, option labels, counts, disabled states, and price min/max. Brand options are exactly Adriatica, Appella, Pierre Ricaud, Rhythm, Romanson, and Royal London. Country options are exactly England, Germany, Japan, South Korea, and Switzerland with RU/KK/EN localization. The country filter is returned once as key `country` with source `brand_country`; `brandCountry` is not returned or accepted as an active public metadata key. | `CatalogFilters` panel; brand pages use the `brand` filter options to resolve canonical backend brand IDs |
| Public CMS page | `GET /api/public/cms/pages/{pageKey}?lang=` | Path `pageKey` such as `home` or `about`; query `lang` accepts frontend `ru/en/kz` and backend `kk` aliases | `PublicCmsPageResponse` with active page metadata, page `updatedAt`, and published blocks ordered by `sortOrder`; each block includes block `updatedAt`, locale media, safe link/settings fields, and localized/RU-fallback translation. About `INSTAGRAM_POST` entries expose only backend-canonical post/reel URLs. Response headers allow a short public cache with stale-while-revalidate. | `PublicApiController.getCmsPage` uses Next `revalidate: 30` with tags `cms` and `cms:{pageKey}`; About uses the published ordered Instagram set or exactly four static non-linked cards when that set is empty |
...
| CRM product list | `GET /api/crm/products?lang=&page=&size=&search=&status=` | Bearer access token; zero-based page; size capped to 100; optional status `ACTIVE`/`DRAFT`/`ARCHIVED`; newest-first default sort | No-store `Page<CrmProductListItemResponse>` with every status when unfiltered. Text search matches ID, model, SKU, status, brand, category code/name, or localized product name and can combine with status. Each row embeds its complete bounded `modelVariantGroup`; all groups for the returned page are resolved by one batch query without changing row totals or pagination. | CRM search/status controls, model-family strips, total/page count, previous/next pagination, focus/mutation refetch |
| CRM product detail | `GET /api/crm/products/{id}?lang=` | Bearer access token | `ProductResponse` including category, brand, collection, images, localized descriptions/names, stock/marketplace links, profile-specific optional details, and a bounded all-status `modelVariantGroup` for sibling edit navigation. Wristwatch details include legacy fields plus dial type/marking, power source, normalized water resistance, shared strap/dial colors, package contents, and a localized feature list. | CRM edit form and sibling product navigation |
| CRM Kaspi product import preview | `POST /api/crm/products/import/kaspi` | Bearer ADMIN/MANAGER token; JSON `{ url, categoryId }`, URL max 2,048 characters; category must resolve to a supported profile. Only HTTPS `kaspi.kz`/`www.kaspi.kz` `/shop/p/...` URLs are accepted; every redirect and resolved address is revalidated. Fail-closed limits default to 12 requests/60 seconds per source, 12/60 per authenticated actor, 60/60 globally, and four concurrent remote fetches. | `KaspiProductImportResponse` with `source`, normalized `sourceUrl`, selected `categoryId`/`categoryProfile`, nullable `values`, mapped fields/characteristics, unmapped characteristics, unresolved dictionary values, and warnings. The endpoint never persists a product. Safe import error codes cover invalid/blocked URLs, fetch status/type/encoding/size/deadline, overload, parse failure, invalid/conflicting values, brand-country mismatch, and category rejection. | Create-only `KaspiImportDialog`; Apply merges resolved values into the existing RHF form, after which the normal product-create endpoint remains authoritative |
| CRM product create | `POST /api/crm/products?lang=` | Essential fields: category, nonblank RU/default `productName.name_ru` (max 255), model (max 255), non-negative integer price/stock, one supported brand, status/type (CRM defaults new items to `DRAFT`). Optional: EN/KZ names, descriptions, collection, photos, marketplace URLs, and partial category-matching detail blocks. Wristwatch details additionally accept nullable `dialTypeId`, `dialMarkingId`, `powerSourceId`, `waterResistanceId`, `strapColorId`, `dialColorId`, `packageContents` (max 500), and up to 50 positive `featureIds`; every field may be omitted and the feature list may be empty. | `ProductResponse`; catalog/profile/dictionary validation errors; audit event `PRODUCT_CREATED` | CRM add product form after the category-first step; all technical fields are optional |
| CRM product bulk create | `POST /api/crm/products/bulk?lang=` | `ProductBulkCreateRequest` with `categoryId`, common defaults and per-row overrides for names/core values/descriptions/profile details/links. Wristwatch common and row values include every new dictionary/color/text field and feature ID list; row values win, including explicit empty feature lists for clearing the inherited set. | `ProductBulkCreateResponse` with created/failed counts, created ids/SKUs, and row-level errors; audit event `PRODUCT_BULK_CREATED` | CRM `/products/bulk` form with profile-specific optional common and row controls |
| CRM product update | `PUT /api/crm/products/{id}?lang=` | Editable core fields plus same-profile category details and marketplace links. Submitted wristwatch detail groups upsert legacy rows, validate active/type-correct dictionaries and shared colors, replace the normalized feature relation, and treat explicit nulls/empty feature IDs as clears. Backend rejects cross-profile detail leakage and invalid dictionary IDs. | `ProductResponse`; audit event `PRODUCT_EDITED` | CRM edit form with optional localized technical fields and creatable references |
| CRM product image upload | `POST /api/crm/products/{id}/images?lang=` | Bearer ADMIN/MANAGER token; explicit multipart request part `files` with one or more decoder-validated image/jpeg, image/png, or image/gif files with matching extensions; max 10 files and 10 MB per file, max 101 MB request. WebP is intentionally not accepted by the current server decoder. | `ProductResponse` with refreshed structured `images`; first upload becomes main when needed; audit event `PRODUCT_IMAGES_UPLOADED` | CRM product create/edit photo section |
...
| CRM marketplace links | `PATCH /api/crm/products/{id}/marketplace-links?lang=` | `{ kaspiUrl, wildberriesUrl }`, optional http/https URLs on the matching marketplace domains; nonblank values are normalized to the canonical Kaspi shop and Wildberries seller URLs | `ProductResponse`; audit event `PRODUCT_MARKETPLACE_LINKS_CHANGED` | CRM edit workflows |
...
| CRM references | `GET /api/crm/references?lang=` | Bearer ADMIN/MANAGER token | Categories, collections, existing watch/interior/accessory dictionaries, shared colors, watch dial types/markings/power sources/water resistances/features, six supported brands, and five supported countries. Options expose localized names plus stable IDs/codes. | CRM individual/bulk product selects and immediate option-cache refresh after inline creation |
| CRM reference create | `POST /api/crm/references/{type}?lang=` | Bearer ADMIN/MANAGER token; allow-listed `{type}` only; JSON `{ ru, kz?, en? }`, RU required, each label max 100. Whitespace is collapsed, empty optional locales fall back to RU, control/HTML-shaped values are rejected, and case/spacing/punctuation/`ё`-normalized duplicates are serialized with transaction-scoped advisory locks. | `CrmReferenceOptionResponse { id, name, code, brandId? }`; `201` on create, `409 REFERENCE_DUPLICATE` on normalized duplicate, `400` on invalid/unsupported input; reference and `REFERENCE_VALUE_CREATED` audit commit atomically. | Reusable `CreatableReferenceFields` dialog; created option is merged and selected without reload |
...
* Deployment validation: use the environment, pull, migration, deploy, smoke, and rollback scripts under `deployment/scripts`; staging/production Compose consumes prebuilt immutable tags. The reusable 8 GB single-VM topology check is `sh deployment/scripts/verify-single-vm-staging.sh infrastructure/compose/compose.single-vm-staging.yml infrastructure/env/gcp-staging.env.example`; Oracle topology validation remains `sh deployment/scripts/verify-oracle-staging.sh infrastructure/compose/compose.oracle-staging.yml infrastructure/env/oracle-staging.env.example`.
* CI: `backend-ci.yml`, `storefront-ci.yml`, and `crm-ci.yml` own component checks/image builds and explicitly record the `next`/`sharp` graph plus the high-severity audit; every direct production CRM build, including `performance-budgets.yml`, supplies the same safe non-secret CI public-build contract. `full-release-gate.yml` is reusable and runs all component/shared infrastructure gates, including Oracle Compose and HTTP-first Nginx validation. `release-images.yml` builds immutable SHA-tagged amd64 images on relevant `main` changes without login, and publishes one `linux/amd64` + `linux/arm64` OCI index per tag only from a manual same-commit full-gate pass. Trusted publication pins Docker GitHub Builder, builds each platform on native `ubuntu-24.04`/`ubuntu-24.04-arm` runners, and assembles the final index without QEMU build execution. Publication adds OCI labels, signed SLSA provenance/SBOM, signed GitHub provenance, index and child digest verification, exact-reference pull/inspection, separate amd64 and QEMU-backed post-publication ARM64 Compose rehearsals, native Java/Node/sharp checks, and a consolidated published manifest. The older `performance-budgets.yml` remains the dedicated combined budget gate.
...
* React Markdown / unified: Both Next.js apps use `react-markdown` 10.1.0, `remark-gfm` 4.0.1, `remark-breaks` 4.0.0, `rehype-raw` 7.0.0, and `rehype-sanitize` 6.0.0 for product descriptions only. Raw HTML is parsed solely before an explicit allow-list sanitizer so `<u>` works without exposing arbitrary database HTML.
...
* Google Compute Engine staging: `compose.single-vm-staging.yml` is the provider-neutral Ubuntu amd64 topology selected by `GCP_STAGING_RUNBOOK.md`; on an `e2-standard-2` it caps long-running containers at 5,440 MiB and binds apps/media only to loopback behind host Nginx. The temporary public routing contract uses the approved `34.18.200.58.sslip.io` shop/CRM/API names, while Google Cloud resources, TLS, secrets, and deployment remain operator-owned.
...
| `SITE_INDEXING_ENABLED` | yes only when deliberate public indexing is approved | Server-only explicit release gate. Only the exact value `true`, together with a valid HTTPS `NEXT_PUBLIC_SITE_URL`, enables indexable metadata/sitemap/robots. Missing, false, malformed, or paired with an invalid origin fails closed to `noindex`. |
| `NEXT_PUBLIC_INSTAGRAM_URL` | no | Optional public Instagram profile URL. It must be an HTTPS `instagram.com` profile path; root, unsafe, or unapproved values are rejected and the UI renders a non-interactive icon/card. |
...
| Variable | Required | Description |
| -------- | -------- | ----------- |
| `NEXT_PUBLIC_CRM_API_BASE` | yes for CRM API calls | Protected backend CRM API base URL exposed to the CRM browser app. It is required in every mode; the current SHA-only staging release contract compiles `https://x7m2q9k4n6p8.vympel.kz/api/crm`, while local development supplies its value through ignored `vympel_crm/.env` rather than a compiled fallback. |
...
| `VYMPEL_RL_*` policy values | no | Endpoint-specific capacity/window overrides for login, registration, refresh/logout, reviews, requests, analytics, quick/catalog reads, the global write burst, and Kaspi import source/actor/global budgets. Defaults are recorded in `STEP_4_SECURITY_ROLLOUT.md` and `application.yml`. |
| `VYMPEL_KASPI_IMPORT_MAX_CONCURRENT_FETCHES` | no | Fair backend bulkhead for in-flight Kaspi remote fetches; default `4`. Excess work fails quickly with `KASPI_IMPORT_BUSY` instead of consuming request threads. |
...
* About Instagram CMS migration: `2026-08-16-01-about-instagram-post-cms.xml` extends the `cms_block.block_type` check with `INSTAGRAM_POST`. Its rollback restores the prior allowed type list; it creates no post rows, so existing environments continue to show the four storefront fallbacks until admins publish CMS posts.
...
* Optional accessory characteristics migration: `2026-08-27-01-optional-product-characteristics.xml` creates one nullable `accessory_details` row per product with optional clasp type, case/insert material foreign keys, tri-state insert presence, color foreign key, and free-text length. It does not rewrite existing products; rollback drops only the new indexes, constraints, and table.
* Kaspi importer migration: `2026-08-31-01-stainless-steel-material.xml` idempotently adds active `STAINLESS_STEEL` in the `CASE` material group with RU/KK/EN labels while leaving `STEEL` unchanged, then replaces `chk_product_kaspi_url_canonical` so validated HTTPS Kaspi `/shop/p/...` source links survive normal product save. The historical canonical merchant URL remains accepted for existing rows and legacy edit behavior. Rollback fails closed if a product references the new material or if a product-specific Kaspi URL would be lost; it never silently deletes referenced data or rewrites product links to a merchant homepage. Existing executed checksums remain valid.
* Watch-product characteristic migration: `2026-09-03-01-watch-product-characteristics.xml` adds one discriminated `watch_attribute_option` dictionary with localized names for dial type, dial marking, power source, and water resistance; adds nullable watch-detail FKs, shared COLOR FKs, and bounded package text; reuses the normalized `watch_feature` / `watch_details_feature` relation; seeds localized canonical values including `BACKLIGHT`; and backfills normalized water resistance without rewriting legacy text. Its guarded rollback aborts if new columns/relations are in use and never cascades through product data; it remains in sequence immediately before the derived-variant index changeset.
* Derived product-model variant migration: `2026-09-04-01-product-model-variants.xml` adds the non-destructive PostgreSQL expression index `idx_product_brand_normalized_model` on `(brand_id, upper(btrim(model)))`. Group membership remains derived at query time; the migration creates no variant table or persistent group identifier. Deployment manifest/rehearsal/runbook boundaries now expect this changeset as the packaged Liquibase tail.
...
* Important tables/entities: `users`, `role`, `user_role`, `refresh_token_session`, `country`, `brand`, `brand_country`, `collection`, `collection_i18n`, `material`, `watch_mechanism`, `gender`, `glass_type`, `stone_inlay`, `category`, `product`, `customer_request`, `product_review`, `product_i18n`, `product_category`, `media`, `media_i18n`, `watch_details`, `watch_attribute_option`, `watch_attribute_option_i18n`, `interior_feature`, `interior_feature_i18n`, `interior_clock_details`, `accessory_details`, `watch_feature`, `watch_feature_i18n`, `watch_details_feature`, `product_description`, `product_description_i18n`, legacy `cms_banner`, `cms_banner_i18n`, `cms_banner_media`, `cms_config`, new `cms_page`, `cms_media`, `cms_block`, `cms_block_translation`, `cms_revalidation_job`, `crm_activity`, `product_analytics_event`.
...
* Marketplace link migration: `2026-07-30-01-canonical-marketplace-links.xml` normalizes populated product Kaspi/Wildberries columns, enforces canonical nullable values with checks, canonicalizes matching CMS external targets with new-tab behavior, and drafts/removes active Ozon CMS destinations without editing older applied migrations.
* Appella/banner migration: `2026-07-30-02-appella-and-contact-banners.xml` updates the existing `brand` row from `apella`/`Apella` to `appella`/`Appella` without changing its primary key or product foreign keys, corrects matching CMS values and old stored brand URLs, restores the static Rhythm CMS-media path from `/rhythm.webp` to `/rhythm.png`, seeds the two new static contact-banner media rows, and assigns the catalog/About blocks to the correct image for all desktop/mobile locale slots. It HALTs on conflicting legacy/canonical rows rather than merging identities; the old `brand_i18n` table is intentionally not referenced because `2026-02-08-01` already removed it.
* Supported catalog-domain migration: `2026-08-02-01-supported-brand-country-domain.xml` is the forward-only source of truth for six brand rows, five country rows, six one-to-one mappings, and RU/EN/KK country labels. It normalizes only clear aliases (`rierrericaud`, `pierre-ricaude`, `adriaica`, `rythm`, and legacy Appella spelling/case), preserves and reassigns supported products/collections, deletes unsupported product roots before unsupported brands/countries so existing cascades clean dependent graphs, aligns interior-clock production countries, and installs identity checks plus deferred exact-domain constraint triggers. Historical applied seed changesets remain immutable and are superseded by this migration.
...
* Product Markdown: Storefront `.product-markdown` keeps the existing responsive 20/18/17px long-copy scale, restrained 32px-or-smaller headings on desktop and 24px-or-smaller headings on mobile, explicit paragraph/list/blockquote/code spacing, wrapping/overflow protection, and visible link focus. CRM `.crm-markdown-editor*` and `.crm-markdown-content` use `--crm-*` tokens, a wrapping toolbar, 260px write/preview surfaces, monospace Markdown authoring text, compact preview headings, and one-column multilingual editor stacking down to 320px.
...
* Instagram card accessibility: CMS-linked About cards use `target="_blank"`, `rel="noopener noreferrer"`, localized accessible names/alt text, and lazy images. Static fallback cards are deliberately non-links. The Home hero has no visual autoplay control; reduced-motion users receive no autoplay, while keyboard-origin focus pauses motion without making pointer-click focus block the required mouseleave resume.
* Responsive breakpoints: Tailwind breakpoints are used directly (`sm`, `lg`, `xl`) with global responsive tokens in `globals.css`: mobile is 0-639px, tablet behavior is 640-1023px, small desktop is 1024-1279px, and desktop is 1280px+. The marketplace grid is one full-width column below `md` and two equal columns with equal-height rows from `md`; its anchors/cards use `w-full min-w-0`, fixed responsive heights, and contained logos so no third-card gap or horizontal overflow remains. The catalog toolbar deliberately keeps its compact layout through 1439px because the measured full accessories row still collides at 1280px; its full inline controls/search start at 1440px. Catalog sort switches from icon-only to its short label at 390px, while catalog hero composition has an explicit 1024-1439px fluid range and 1440px large-desktop minimum. The public storefront uses `.responsive-page-x`, `.responsive-section-gap`, `.responsive-product-grid`, `.responsive-home-banner-*`, `.responsive-page-banner-image`, `.goods-carousel-*`, `.mobile-bottom-nav-item`, `.product-card-*`, and `.vympel-toast-*` to keep gutters, section rhythm, tall mobile hero banners, product rails, bottom nav safe area, product grids, large banners, card text/badges, and toast actions stable down to 320px. Responsive QA must include the collision band around 900/1000/1024/1100/1280px as well as 1440, 768, 480, 430, 414, 390, 375, 360, and 320px.
* Asset locations: Static images are in `vympel_front/public`; icons are React components in `vympel_front/src/assets/icons`. Next App Router serves the multi-size black/white VYMPEL `V` monogram from `vympel_front/src/app/favicon.ico`; no parallel metadata icon declaration is used. The CMS seed stores public assets as `PUBLIC_PATH` media rows; newly uploaded CMS images use MinIO/S3 object keys and public links, and public rendering appends timestamp-based query params so replaced CMS media does not reuse stale browser/CDN entries. The catalog contact block uses `/contact-banner-catalog.png`; the About cooperation block uses `/contact-banner-about-page.png`; product contact keeps `/contact_banner.png`. The home accessories product rail uses `/accessories-banner.png`. The About hero remains `about-us-banner.png`; its zero-CMS Instagram fallback is exactly `/insta-1.webp`, `/insta-2.png`, `/insta-3.webp`, and `/insta-4.webp`. CMS-managed Instagram cards reuse ordinary CMS media. Marketplace logos are `kaspi.png` and `wb.png`. The home brand carousel and corresponding static CMS-media fallback map Rhythm to `/rhythm.png` while other brand cards keep their slug-based WebP paths; the obsolete `rhythm.webp` storefront asset is removed. Brand-page banners remain mapped in `src/config/brandPages.ts`, including `Appella_brand_banner.jpg`/`Appella_catalog_banner.jpg` and `Rhythm_brand_banner.webp` as the separate Rhythm brand-page hero fallback. Delivery/Payment public info pages reuse `shop.png` through `StoreLocationBlock`.
...
* About Instagram design clarification: the four design-reference cards keep the same proportions/rounded icon treatment, but the rail no longer autoplays. CMS posts may replace all four static cards; when CMS has zero usable published posts, the exact mixed WebP/PNG fallback set listed above renders without links.

## Testing Strategy

* Frontend tests: Public Vitest covers recommendation behavior, local product summaries, 429 parsing, CMS revalidation, canonical routes, metadata, carousels, product Markdown, and populated/empty optional watch specs; the current storefront suite runs 37 files / 155 tests. CRM Vitest covers auth/session behavior, CMS feedback, brand-country derivation, Markdown, Kaspi preview/apply/category isolation, creatable reference success/cancel/double-submit/conflict/XSS behavior, and individual/bulk product payloads; the current CRM suite runs 15 files / 76 tests. Both apps require lint, typecheck, finite tests, npm audits, and production builds.
* Backend tests: The Java 17 suite currently runs 62 suites / 257 tests (one skipped environment-gated rehearsal) and covers the existing security/catalog/CMS/deployment baseline plus watch characteristic migration structure, dictionary type/activity validation, advisory-lock duplicate creation, transactional audit behavior, zero/one/many feature replacement, individual/bulk round-trips, conservative Kaspi mapping including live leaf function/date labels, and fail-closed rollback/boundary contracts.
* Integration tests: `SupportedCatalogMigrationTest` runs the full fresh Liquibase chain on disposable PostgreSQL 16 and proves the exact 6/5/6 domain, 15 localized labels, DB rejection of unsupported/drift writes, clear alias contract, and FK-safe root deletion. `CmsMediaDryRunIntegrationTest` is a read-only configured-PostgreSQL reconciliation check; on 2026-07-17 it observed 24 media rows and 5 eligible zero-reference candidates without changing the row count. `RefreshSessionMigrationTest` rehearses the fresh PostgreSQL 16 chain, all six CMS reference slots, Step 7 constraints, invalid writes, and concurrency. Environment-gated `Step7ExternalDatabaseRehearsalTest` finite-migrates a restored/current external database and verifies constraints/data plus direct rejection without becoming a normal live dependency.
...
* Storefront accessibility/SEO audit evidence: `docs/audits/FRONTEND_ACCESSIBILITY_SEO_AUDIT.md` and `CODEX_FRONTEND_ACCESSIBILITY_SEO_IMPLEMENTATION_PROMPT.md` pin commit `1786e855e7a0b6a4797cdfae651e0dfe206709a2`, cover 17 public route/endpoint patterns and three locales through 10 checkpoints, and hand off 28 findings (1 P0, 14 P1, 13 P2) without changing storefront source. The P0 is public temporary-host indexability; implementation must begin from the audit's fail-closed staging/domain contract and re-check current code drift.

## Known Architectural Decisions

* Storefront indexing is a two-input, server-owned release decision: `SITE_INDEXING_ENABLED=true` is necessary but not sufficient; an approved origin-only HTTPS `NEXT_PUBLIC_SITE_URL` is also required. Staging/default builds fail closed at headers, metadata, robots, and sitemap.
* Catalog SEO identity is path-first: clean category pages use `/catalog/{categoryCode}`, clean `page=n` pages self-canonicalize, and search/facets/non-default sort use `noindex, follow` with a clean canonical. Legacy `categoryCode` query URLs permanently redirect instead of coexisting as duplicates.
* Product/catalog initial content is server-owned. Product metadata and UI share a cached loader; catalog HTML is seeded with the first API page. Confirmed absence maps to 404, while transient dependency failure must never masquerade as 404 or fabricate content.
* Structured data is emitted only from current API facts and only on indexable public states. Do not invent reviews, ratings, stock, prices, organization profiles, social URLs, or product imagery.
* Public disclosures/lightboxes use conditional rendering or Radix dialogs with Escape, focus return, focus trapping, and background scroll/inert management. Carousel slides outside `slidesInView` are `aria-hidden` and `inert`; an empty observer result falls back to the selected snap so a visible slide never becomes inaccessible.
* Route locale and document/Intl locale are separate: public paths remain `ru`/`kz`/`en`, while HTML/Intl use `ru`/`kk`/`en` and `ru-RU`/`kk-KZ`/`en-US`.
...
* Product rich text is Markdown, not stored HTML or editor JSON: the backend/API contract remains the existing multilingual strings in `content_md`. CRM toolbar operations produce Markdown plus sanitized `<u>` tags, storefront rendering is server-owned, CRM preview rendering is lazy client-side, and both apps share the same explicit safe-tag/protocol schema. A database migration is not required for formatting changes.
...
* Supported catalog domain is closed and authoritative: `SupportedBrandCountry` and `SupportedCatalogDomainService` mirror the DB's exact six mappings for application validation and reference projection. CRM and storefront can render localized values, but they cannot invent, remap, or independently submit a country. Database checks/deferred triggers protect direct writes, while the forward-only migration owns legacy normalization and unsupported graph cleanup.
...
* Carousel autoplay is exceptional and Home-only: the multi-slide Home hero is the only carousel that imports Embla Autoplay. It starts automatically when reduced motion is not requested, has no visual Play/Pause control, pauses while the mouse is anywhere over the carousel or keyboard-origin focus remains inside, and resumes after mouseleave, focus exit, touch/mouse drag completion, or Embla reinitialization. Pointer-click focus must not block mouseleave resume; visibility restoration reasserts lifecycle authority. Brands, About Instagram, product rails/recommendations, and product gallery remain manual swipe/arrow/dot experiences.
...
* Centralized public navigation: Public internal links and CTA destinations should be built through `vympel_front/src/config/routes.ts`. Semantic category links use `/catalog/{categoryCode}`, semantic filter links append backend filter keys/values such as `gender=1` to the clean path, search links use `/catalog?search=...`, and brand catalog links should use a backend-resolved brand filter value when filtering by brand. Default `page=1` and default sort are omitted; legacy `categoryCode` query URLs redirect to the path form.
...
* About Instagram URLs have a dedicated trust boundary: accepted input is HTTPS on exact host `instagram.com` or `www.instagram.com` and exact path `/p/{id}` or `/reel/{id}` with an alphanumeric/underscore/hyphen id. Query/fragment are stripped into `https://www.instagram.com/.../`; ports, userinfo, fake hosts, encoded path tricks, extra segments, stories, profile/root URLs, and other schemes are rejected. Backend policy is authoritative; CRM/storefront mirrors are defense in depth.
...
The workspace is one root Git monorepo with three independent image boundaries: `ghcr.io/zxzxz143/vympel-backend`, `ghcr.io/zxzxz143/vympel-storefront`, and `ghcr.io/zxzxz143/vympel-crm`. Each manual publication tag is one OCI index containing exactly one Linux amd64 and one Linux arm64 application manifest, plus optional attestation manifests. `compose.yml` remains the local source-build stack with PostgreSQL 16, Redis 7.4, and MinIO. `infrastructure/compose/compose.staging.yml` and `compose.production.yml` consume immutable `${REGISTRY}/<image>:${RELEASE_TAG}` references and expect data services/TLS to be external. `compose.single-vm-staging.yml` owns bounded persistent PostgreSQL/Redis/MinIO services for a provider-neutral 2 vCPU/8 GB host, runs Liquibase through a finite job, keeps infrastructure ports unpublished, and binds apps/media only to loopback behind host Nginx. `compose.oracle-staging.yml` preserves the separately verified ~2 OCPU/~12 GB ARM64 Oracle variant.

Both Next apps use `output: "standalone"` and run as the image's non-root `node` user. Their runtime stages apply the fixed Alpine `libcrypto3`/`libssl3` packages and remove the unused npm/npx installation before copying the standalone application, so build tooling and its advisory surface do not ship in production. The Spring Boot image runs as UID/GID 10001. Deployment starts the finite `migrate` service before backend replicas; normal replicas have Liquibase disabled. Nginx is the sole published service and routes configurable storefront, CRM, and API domains while keeping readiness and protected Actuator endpoints internal.

`deployment/scripts` provides environment/public-build validation, Oracle topology validation, fail-closed historical Liquibase compatibility, image pull, multi-platform published-image inspection, pre-publication and digest-complete manifest generation, migration verification, bounded health polling, smoke checks, deploy, backup evidence checks, and immutable image rollback. `verify-frontend-bundle-urls.sh` is the reusable flattened-bundle guard invoked for both frontend platform children by `verify-published-images.sh`. Cross-platform PowerShell under `deployment/rehearsals` owns disposable PostgreSQL backup/restore, signed CMS retry/freshness, and Nginx routing proofs; published-image overrides prove exact GHCR images as amd64 and ARM64 against namespaced disposable local services. The migration container publishes no port, disables scheduling, and exits after Liquibase completes. Database rollback is deliberately absent. Full commit SHA is always an application image tag; successful publication records all three index digests, six platform child digests, exact frontend public build configuration, latest Liquibase change, and remaining provider decisions. `deployment/releases/v1.0.0-rc.2.yml` remains the immutable amd64-only baseline and is never moved. RC.3 at `a87c2883023b6ef6d8ed56f33856a242963a0c51` is also immutable but incomplete: backend and CRM indexes exist, storefront is absent, and no runtime or consolidated-manifest gate passed. RC.4 at `ac924fae45e94911c65c95a18f10e8c53a587c3a` is immutable and has no image tags because its distributed builder stopped on context-relative Dockerfile resolution before registry writes. RC.5 at `8bc963babe17b7c4e5177fa19831a8eb310880cf` is immutable and incomplete: native storefront and CRM indexes exist under both RC and full-SHA tags, backend tags are absent because Dockerfile frontend 1.7.1 rejected the builder's Git-checksum capability, and downstream attestation/runtime/manifest gates did not run. RC.6 at `d422e38e233f00f55e2c96a37d8600c5b316fef9` has complete native amd64/ARM64 indexes for all three apps under matching RC/full-SHA tags, but its separate final-index attestation jobs lacked registry login and therefore metadata/runtime/consolidated-manifest gates did not run; it is immutable and non-deployable. RC.7 at `477d32b36e817bb92b7000e94851e5756a186d9c` is the first complete amd64/ARM64 three-image publication: native builds, signed final indexes, metadata, exact-image amd64/ARM64 runtime rehearsals, and consolidated manifest all passed in run `30231792115`. Its committed `deployment/releases/v1.0.0-rc.7.yml` records acknowledged `.invalid` public origins, and a direct amd64 bundle scan found the former local media pattern in the storefront server chunk, so it remains evidence-only. RC.8 at `aadfabfd23a4196bc385ae41a0f3b8719172ffbc` is the temporary public GCP staging candidate: run `30400324979` published and verified all three amd64/ARM64 indexes using the exact sslip.io build contract, passed both platform runtime and bundle URL gates, and emitted the committed `deployment/releases/v1.0.0-rc.8.yml`. RC.9 at `7167aa31618d6c090c8c5b04394c04ab03866dc4` publishes the banner/carousel/Appella application update under the same approved sslip.io contract: run `30557528961` passed signed indexes, metadata, both exact-image runtime paths, bundle scans, and manifest generation after one transient backend amd64 signing retry. `deployment/releases/v1.0.0-rc.9.yml` records all registry digests and the actual final Liquibase changeset `2026-07-30-02-update-contact-banner-media`. No cloud deployment ran.

RC.10 at `9a012b4c1258c29ebd6b6c4ff245018f846d1a27` publishes the restored storefront navigation-progress behavior under the approved sslip.io staging contract. Release Images run `30753618665` passed all release gates, native amd64/ARM64 builds, signed final indexes, exact-image runtime and bundle checks, and consolidated manifest generation without retries. `deployment/releases/v1.0.0-rc.10.yml` records all three index digests, six child digests, exact public build values with telemetry disabled, and the tagged source's actual final Liquibase changeset. Its RC and full-SHA registry tags resolve to the same indexes, no `latest` tags were created, and no cloud deployment ran.

RC.11 at `d6cc31e0ed7d9d1a08337f7fd2633ad8f461e3ff` publishes the exact six-brand/five-country catalog domain under the unchanged RC.10 sslip.io staging contract. Release Images run `30762647549` passed all publication gates, native amd64/ARM64 builds, signed final indexes, registry non-overwrite checks, both exact-image runtime and migration paths, bundle scans, and consolidated manifest generation without retries. `deployment/releases/v1.0.0-rc.11.yml` records all three index digests, six child digests, exact public build values with telemetry disabled, and the actual final Liquibase changeset `2026-08-02-01-supported-brand-country-domain`. Its RC and full-SHA tags resolve to identical indexes, all three `latest` refs are absent, RC.10 remains unchanged, and no cloud deployment ran.

RC.12 at `f3c8196c93458af9d231776803fb361dca8175d6` is an immutable failed publication attempt for product Markdown. Release Images run `32866239945` stopped inside the pre-publication full release gate: CRM hit the CI-only React refs lint rule, the backend image exposed newly actionable Spring/Jackson advisories through Spring Boot 4.0.2, and the pinned Node runtime carried npm packages plus outdated Alpine OpenSSL packages. Publication preflight never ran, so no RC.12 image tags or deployment were created.

The CMS retry rehearsal treats the immediate outbox observation as a concurrent transition: after the API reports `FAILED_RETRY_SCHEDULED`, the row may still be `RETRY` or may already be claimed as `PROCESSING`, but it must have at least one attempt. The later assertion still requires terminal `SUCCEEDED` with at least two attempts and fresh storefront HTML. The reverse-proxy rehearsal polls the newly published Nginx health port with a finite deadline and treats transient connection refusal/reset as not-ready; all routing and policy assertions remain fail-fast after readiness.
...
Provider-neutral deployment templates and runbooks live under `infrastructure`, `deployment`, and `docs/deployment`. `GCP_STAGING_RUNBOOK.md` documents manual Compute Engine, Docker, GHCR, Nginx, DNS/TLS, ADMIN, update, and rollback steps without authorizing cloud provisioning or deployment. The GCP environment template remains pinned to RC.8's exact full SHA and carries the temporary sslip.io public/runtime routing contract; publishing RC.9 did not mutate that deployment state. Publication metadata records all exact `NEXT_PUBLIC_*` release values, including disabled telemetry, and frontend CI plus exact platform images use the shared bundle guard for reserved `.invalid`, localhost, and loopback URLs before the manifest is emitted. Local proof covers SEO behavior, PostgreSQL backup/restore, signed CMS freshness and retry, Prometheus syntax, and reverse-proxy routing. RC.8 and RC.9 GHCR proof covers matching RC/full-SHA indexes, exact platform-digest pulls, amd64/ARM64 non-root/OCI/no-secret-metadata inspection, signed attestations, Liquibase/readiness, native Java/Node/sharp behavior, image optimization, bundle URL scans, and isolated healthy runtimes. Production remains blocked for any target containing the unrecoverable historical Liquibase row until accountable external acceptance, and on provider/data-service/secrets/public-TLS/trusted-proxy/monitoring/real-staging evidence.

The 2026-08-21 SHA-only preview publication contract compiles the storefront for `https://preview.vympel.kz`, public API/media for `https://api.vympel.kz`, CRM API for the same protected CRM origin at `https://x7m2q9k4n6p8.vympel.kz/api/crm`, staging environment, exact commit release metadata, disabled telemetry, and explicit `SITE_INDEXING_ENABLED=false`. No `crm.vympel.kz` or public `api.vympel.kz/api/crm` contract is introduced. Future host Nginx routes the protected CRM `/` to the CRM frontend and `/api/crm/*` to the backend; UI plus login/refresh/logout use VM-only Basic Auth, while remaining CRM API paths have Nginx Basic Auth disabled so their Bearer header reaches Spring. Backend JWT/RBAC, trusted-origin checks, rate limiting, and the host-only Secure/HttpOnly/Lax refresh cookie remain unchanged. `deployment/scripts/verify-published-images.sh` requires the exact published CRM API base in both platform bundles and rejects the superseded sslip API origin; `verify-preview-indexing.sh` still requires global noindex, no canonical/alternate/social discovery metadata, no advertised sitemap, and no sitemap locations. Publication uses only the full 40-character SHA and creates no RC or mutable alias. The checked-in GCP env remains historical sslip deployment state until a separately authorized VM/Nginx/DNS/TLS change; the old sslip CRM origin is rollback-only and must later be removed from CORS/routing after verification.
