...
Mobile catalog overlay coordination
  -> category/filter/sort/search trigger passes its HTMLElement to CatalogOverlayProvider
  ...
  -> close clears the active overlay, waits for the 220ms exit transition, restores bottom navigation, and returns focus to the connected trigger
```
...
User focuses SmartSearch in Navigation, CatalogPage, or ProductPage
  -> React Hook Form owns the search query field while local state owns overlay open/status/results
...
`deployment/scripts` provides environment validation, fail-closed historical Liquibase compatibility, image pull, migration verification, bounded health polling, smoke checks, deploy, backup evidence checks, and immutable image rollback. `deployment/rehearsals` owns disposable PostgreSQL backup/restore, signed CMS retry/freshness, Liquibase gate, and Nginx routing proofs. The migration container publishes no port, disables scheduling, and closes as soon as the verification runner confirms Liquibase state; it retains the normal web application type because the current security configuration requires `HttpSecurity` during context creation. Database rollback is deliberately absent. The five required CI workflows split backend/storefront/CRM checks, manual guarded image release, and the full release gate; the existing performance-budget workflow remains separate. Full commit SHA is the application image tag and the release gate emits a commit-specific manifest artifact with registry digests pending until publication.
...
2026-07-22 - Added the RC SEO/discovery contract, CRM noindex policy, observable runtime signals, historical Liquibase fail-closed gate, disposable backup/CMS/proxy rehearsals, build-time canonical URL, and commit-specific release-manifest evidence.
