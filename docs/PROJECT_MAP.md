...
Public product detail or bounded CRM product page
  ...
  -> window functions keep current-first/id ordering, exact total, and at most 24 returned variants
...
2026-09-04 - Added derived brand/model/category-profile product groups across backend, storefront, and CRM; documented the bounded batch projection, exact public visibility rules, expression index, collision-safe SKU creation, model-driven regrouping, direct thumbnail image loading, and exact Kaspi model promotion verified in the local three-variant round-trip.
