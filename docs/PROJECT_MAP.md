...
Provider-neutral deployment templates and runbooks live under `infrastructure`, `deployment`, and `docs/deployment`. `GCP_STAGING_RUNBOOK.md` documents manual Compute Engine, Docker, GHCR, Nginx, DNS/TLS, ADMIN, update, and rollback steps without authorizing any external action. Local proof now covers SEO behavior, PostgreSQL backup/restore, signed CMS freshness and retry, Prometheus syntax, and reverse-proxy routing. RC.7 GHCR proof covers exact platform-digest pulls, amd64/ARM64 non-root/OCI/no-secret-metadata inspection, signed attestations, Liquibase/readiness, native Java/Node/sharp behavior, image optimization, and isolated healthy runtimes. Production remains blocked for any target containing the unrecoverable historical Liquibase row until accountable external acceptance, and on provider/final-domain/data-service/secrets/public-TLS/trusted-proxy/monitoring/final-origin image/real-staging evidence.

## Last Updated

2026-07-27 - Added the provider-neutral 8 GB single-VM/GCP staging topology, environment and Nginx contracts, manual runbook, Java memory budget, and non-local frontend localhost exclusion; RC.7 remains evidence-only.
