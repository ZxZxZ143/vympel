...
* Instagram card accessibility: CMS-linked About cards use `target="_blank"`, `rel="noopener noreferrer"`, localized accessible names/alt text, and lazy images. Static fallback cards are deliberately non-links. The Home hero's compact 44x44 icon is the only autoplay control and uses localized pause/resume labels.
...
* Carousel autoplay is exceptional and Home-only: the Home hero is the only carousel that imports Embla Autoplay. Its compact icon control lives inside the existing navigation, exposes localized pause/resume labels, stops on pointer/focus/manual navigation, and is absent when reduced motion is requested. Brands, About Instagram, product rails/recommendations, and product gallery remain manual swipe/arrow/dot experiences with no large start/stop text pills.
...
2026-08-21 - Moved the SHA-only CRM build contract to `x7m2q9k4n6p8.vympel.kz`, documented the same-origin Basic/Bearer ingress split and rollback-only sslip boundary, and added exact-image CRM bundle origin verification without changing VM state.
