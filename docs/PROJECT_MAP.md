...
* Frontend tests: Public Vitest covers recommendation behavior, one-request/in-flight-deduplicated batch summary, batch snapshot/unavailable merging, 429 parsing, CMS revalidation signature/payload/target mapping, exact six canonical brand routes, product marketplace-action absence/28px spacing, localized metadata sanitization/composition, and the Home hero autoplay lifecycle/interaction/cleanup policy. The current storefront suite runs 35 files / 146 tests. The isolated production status/content matrix also validates favicon 200/ICO, RU/KZ/EN route-specific title/description on Home/Catalog/Category/Product/Brands/Brand/About/Delivery/Payment/Guarantee, real product copy, marketplace anchor absence, canonical/hreflang, and matching OG/Twitter metadata. CRM Vitest covers auth/session behavior, explicit CMS partial-success feedback, and canonical brand-country derivation. Both apps require lint and production builds; public additionally runs typecheck, security, budgets, and sharp dependency gates.
