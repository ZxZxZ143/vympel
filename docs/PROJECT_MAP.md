...
| Public route helpers | `vympel_front/src/config/routes.ts`, `src/config/brandRoutes.ts`, `src/app/[locale]/brands/[brandSlug]/page.tsx` | Central source for public internal hrefs, semantic catalog/category/filter presets, contact links, marketplace links, and product/brand/cart/favorites/about/info-page route builders. `MARKETPLACE_LINKS` contains only the canonical Kaspi shop and Wildberries seller destinations; `resolveMarketplaceHref` upgrades legacy CMS links and blocks Ozon domains. Brand routes accept and emit only `adriatica`, `appella`, `pierre-ricaud`, `rhythm`, `romanson`, and `royal-london`; old misspelled aliases are normalized in the forward database migration, not redirected at runtime. Category links use `categoryCode` query params, clear incompatible filters by construction, and reset `page=1`; men/women watch links use the backend `gender` filter values; accessories keep the `ACCESSORIES` category and use component-owned `gender` query toggles instead of child category routes, including a clean all-accessories state; kids watches use `WATCH_KIDS`; `routes.brands()` owns the all-brands page. `CONTACT_LINKS.instagram` is currently a centralized placeholder until the real Vympel Instagram URL is known. | `routes`, `catalogLinks`, `PUBLIC_CATEGORY_CODES`, `SEEDED_FILTER_VALUES`, `CONTACT_LINKS`, `MARKETPLACE_LINKS`, `resolveMarketplaceHref`, `PUBLIC_BRANDS` |
...
* Frontend tests: Public Vitest covers recommendation behavior, one-request/in-flight-deduplicated batch summary, batch snapshot/unavailable merging, 429 parsing, CMS revalidation signature/payload/target mapping, and the exact six canonical brand routes. CRM Vitest covers auth/session behavior, explicit CMS partial-success feedback, and canonical brand-country derivation. Both apps require lint and production builds; public additionally runs typecheck and the isolated production status/content matrix.
...
2026-08-02 - Enforced the exact six-brand/five-country catalog domain and recorded the successful immutable RC.11 multi-architecture publication, verified registry digests, corrected final migration metadata, and no-deployment boundary.
