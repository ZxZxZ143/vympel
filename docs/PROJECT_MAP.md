...
| Kaspi product preview importer | `services/marketplace/kaspi/*`, `dtos/product/KaspiProductImport*.java`, `exceptions/ProductImportException.java` | Fetches one allow-listed HTTPS Kaspi product page with explicit redirect revalidation, DNS/private-network rejection, direct no-proxy HTTP, timeouts, content-type checks, and a 2 MiB streaming cap. It parses JSON-LD first with constrained embedded/semantic DOM fallbacks, resolves fields through category-profile-specific exact/normalized/alias dictionaries, distinguishes unresolved from unmapped characteristics, and returns a safe preview without calling product persistence. | `KaspiUrlGuard`, `SecureKaspiPageFetcher`, `KaspiProductParser`, `KaspiCharacteristicMapper`, `KaspiProductImportService`, `KaspiProductImportResponse` |
...
| CRM Kaspi product import preview | `POST /api/crm/products/import/kaspi` | Bearer ADMIN/MANAGER token; JSON `{ url, categoryId }`, URL max 2,048 characters; category must resolve to a supported profile. Only HTTPS `kaspi.kz`/`www.kaspi.kz` `/shop/p/...` URLs are accepted; every redirect and resolved address is revalidated. Dedicated fail-closed rate limit is 12 requests per 60 seconds. | `KaspiProductImportResponse` with `source`, normalized `sourceUrl`, selected `categoryId`/`categoryProfile`, nullable `values`, mapped fields/characteristics, unmapped characteristics, unresolved dictionary values, and warnings. The endpoint never persists a product. Safe import error codes cover invalid/blocked URLs, fetch status/type/size/timeouts, parse failure, and category rejection. | Create-only `KaspiImportDialog`; Apply merges resolved values into the existing RHF form, after which the normal product-create endpoint remains authoritative |
...
* Frontend tests: Public Vitest covers recommendation behavior, local product summaries, 429 parsing, CMS revalidation, canonical routes, metadata, carousel behavior, and product Markdown rendering/sanitization/plain-line compatibility; the current storefront suite runs 36 files / 150 tests. CRM Vitest covers auth/session behavior, CMS feedback, canonical brand-country derivation, Markdown behavior, the Kaspi API request contract, create-only dialog states, preview readability, cancel/apply behavior, profile-safe RHF merging, manual edits after import, and the unchanged normal submit path; the current CRM suite runs 13 files / 63 tests. Both apps require lint, typecheck, finite tests, npm audits, and production builds. Dependency changes additionally require clean npm 10.9.8 Linux `npm ci` through the pinned Docker dependency stages.
* Backend tests: The Java 17 suite covers the Step 4/5/6 baseline plus Step 7 product activation/final-image rules, collision-free media/CMS reordering, stable constraint error codes, fresh-chain constraints, direct invalid SQL, concurrent stock/media/CMS writes, ADMIN bootstrap validation/creation/idempotency/collision/log-safety behavior, authoritative supported-domain validation, individual/bulk mismatches, interior-country derivation/realignment, and Kaspi URL/redirect SSRF defense, parsing fallbacks, category-specific dictionary mapping, non-persistence, rate-limit routing, and migration contracts.
...
2026-09-01 - Added the protected non-persisting Kaspi product preview pipeline, create-only CRM import/apply flow, strict remote-fetch and rate-limit contract, stainless-steel/source-link Liquibase changes, deterministic coverage, and a verified healthy local Compose stand.
