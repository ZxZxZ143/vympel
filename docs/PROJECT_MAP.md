...
| Home screen | `vympel_front/src/screens/HomePage/index.tsx`, `src/components/HomePage/bannerCarousel/*`, `src/components/HomePage/ProductCarouselSection/index.tsx` | Composes navigation, tall responsive hero/banner carousel, benefits, new arrivals, brands, categories, an accessories rail immediately after categories, mobile-redesigned philosophy/story block, and marketplaces. The multi-slide hero is the storefront's only autoplay carousel: it starts automatically when reduced motion is not requested, pauses across whole-carousel mouse hover and keyboard-origin descendant focus, resumes after leave/blur and drag/touch completion, and has no visual Play/Pause control. A shared lifecycle owns Embla reInit, visibility restoration, listener/frame cleanup, pointer-versus-keyboard focus semantics, and ignores the shared Carousel's programmatic root-focus fallback after a dragged focused slide becomes hidden. New arrivals and accessories share the server-rendered `ProductCarouselSection`, which loads real category products through the existing by-code API, preserves backend availability-first/newest ordering, maps locale-aware product/catalog routes, hides genuinely empty sections, and delegates card/swipe/arrow/banner behavior to `GoodsCarouselWithImage`. The accessories rail uses `/accessories-banner.png`; the new-goods banner remains CMS-first with its existing static fallback. The marketplace section contains only Kaspi and Wildberries: one full-width column on narrow screens and two equal-height columns from `md`. Home product rails keep the controlled desktop banner layer, banner hidden on tablet/mobile when it becomes visual noise, and non-overlapping arrows. | default async `HomePage`, `ProductCarouselSection` |
