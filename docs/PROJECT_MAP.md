...
Provider-neutral deployment templates and runbooks live under `infrastructure`, `deployment`, and `docs/deployment`. `GCP_STAGING_RUNBOOK.md` documents manual Compute Engine, Docker, GHCR, Nginx, DNS/TLS, ADMIN, update, and rollback steps without authorizing cloud provisioning or deployment. The GCP environment template now carries the exact temporary sslip.io public/runtime routing contract. Publication metadata records all exact `NEXT_PUBLIC_*` release values, including disabled telemetry, and frontend CI plus exact platform images use the shared bundle guard for reserved `.invalid`, localhost, and loopback URLs before the manifest is emitted. Local proof covers SEO behavior, PostgreSQL backup/restore, signed CMS freshness and retry, Prometheus syntax, and reverse-proxy routing. RC.7 GHCR proof covers exact platform-digest pulls, amd64/ARM64 non-root/OCI/no-secret-metadata inspection, signed attestations, Liquibase/readiness, native Java/Node/sharp behavior, image optimization, and isolated healthy runtimes. Production remains blocked for any target containing the unrecoverable historical Liquibase row until accountable external acceptance, and on provider/data-service/secrets/public-TLS/trusted-proxy/monitoring/real-staging evidence.

## Last Updated

2026-07-29 - Prepared the exact temporary GCP sslip.io routing contract, complete public-build manifest variables, and amd64/ARM64 published-bundle placeholder/loopback scans for the next immutable RC.
