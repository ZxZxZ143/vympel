...
Provider-neutral deployment templates and runbooks live under `infrastructure`, `deployment`, and `docs/deployment`. Local proof now covers SEO behavior, PostgreSQL backup/restore, signed CMS freshness and retry, Prometheus syntax, and reverse-proxy routing. GHCR publication proof covers exact digest pulls, amd64/non-root/OCI/no-secret-metadata inspection, attestations, and an isolated healthy runtime. Production remains blocked for any target containing the unrecoverable historical Liquibase row until accountable external acceptance, and on provider/final-domain/data-service/secrets/public-TLS/trusted-proxy/monitoring/final-origin image/real-staging evidence.

## Last Updated

2026-07-27 - Recorded immutable post-publication RC.6 and added explicit GHCR authentication to final-index attestation jobs.
