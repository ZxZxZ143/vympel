...
| CRM product management and Markdown editor | `vympel_crm/src/features/products/*`, `src/shared/markdown/*` | Pageable newest-first no-store product management plus category-first create/edit, inline collections, images, quick edits, and bulk creation. Watch, interior-clock, and accessory technical characteristics are optional and profile-scoped; accessories expose clasp type, case/insert materials, tri-state insert presence, color, and free-text length in individual and bulk forms. The RU/EN/KZ product-description fields use the shared lightweight `MarkdownEditor`: localized formatting toolbar, Write/Preview modes, selection-safe bold/italic/underline/heading/list/link/blockquote transforms, automatic list continuation, multiline/plain-paste compatibility, and the same sanitizer-backed lazy preview as the storefront contract. RHF remains the sole value owner; existing Markdown loads and saves without conversion to a proprietary format. | `ProductListView`, `ProductForm`, `MarkdownEditor`, `MarkdownContent`, editor transform helpers, `BulkProductCreateView`, `brandCountryFor` |
...
* Frontend tests: Public Vitest covers recommendation behavior, local product summaries, 429 parsing, CMS revalidation, canonical routes, metadata, carousel behavior, and product Markdown rendering/sanitization/plain-line compatibility; the current storefront suite runs 36 files / 150 tests. CRM Vitest covers auth/session behavior, CMS feedback, canonical brand-country derivation, Markdown toolbar transforms/list continuation, and exact RU/EN/KZ Markdown save/reopen round trips; the current CRM suite runs 12 files / 48 tests. Both apps require lint, typecheck, finite tests, npm audits, and production builds. Dependency changes additionally require clean npm 10.9.8 Linux `npm ci` through the pinned Docker dependency stages.
* Backend tests: The Java 17 suite covers the Step 4/5/6 baseline plus Step 7 product activation/final-image rules, collision-free media/CMS reordering, stable constraint error codes, fresh-chain constraints, direct invalid SQL, concurrent stock/media/CMS writes, ADMIN bootstrap validation/creation/idempotency/collision/log-safety behavior, authoritative supported-domain validation, individual/bulk mismatches, and interior-country derivation/realignment.
