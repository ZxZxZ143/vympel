...
| CRM product management, Kaspi preview, and Markdown editor | `vympel_crm/src/features/products/*`, `src/shared/markdown/*` | Pageable newest-first no-store product management plus category-first create/edit, inline collections, images, quick edits, and bulk creation. The create-only Kaspi action appears after category selection, opens an accessible localized dialog, validates a product URL, displays mapped/unmapped/unresolved preview results, and applies only present resolved values into the existing RHF form; category/profile stays fixed, unresolved or absent values preserve manual entries, stale inline-collection state is cleared when the imported brand changes, and the ordinary create submission remains the only persistence path. The preview dialog is a viewport-bounded flex column: header/footer/close stay fixed, only its focusable named body scrolls, long marketplace text wraps without horizontal overflow, background scrolling is locked, and focus trap/Escape/close/restore behavior remains deterministic down to 320px and short-height viewports. Watch, interior-clock, and accessory technical characteristics are optional and profile-scoped; accessories expose clasp type, case/insert materials, tri-state insert presence, color, and free-text length in individual and bulk forms. The RU/EN/KZ product-description fields use the shared lightweight `MarkdownEditor`: localized formatting toolbar, Write/Preview modes, selection-safe bold/italic/underline/heading/list/link/blockquote transforms, automatic list continuation, multiline/plain-paste compatibility, and the same sanitizer-backed lazy preview as the storefront contract. | `ProductListView`, `ProductForm`, `KaspiImportDialog`, `applyKaspiImportPreview`, `MarkdownEditor`, `MarkdownContent`, `BulkProductCreateView`, `brandCountryFor` |
...
| Product services | `services/product/*`, `services/catalog/SupportedBrandCountry.java`, `services/catalog/SupportedCatalogDomainService.java`, `services/marketplace/MarketplaceUrlPolicy.java`, `productName/*`, `productDescription/*`, `watchDetail/*`, `accessoryDetail/*`, `categoryProduct/*`, `mappers/product/ProductDescriptionMapper.java` | Product creation/read models, bulk orchestration, SKU generation, localized names/descriptions, category links, category-profile validation, canonical Kaspi/Wildberries URL enforcement, and one authoritative immutable six-brand/five-country domain. Every individual or bulk product create/update validates the selected brand and any submitted interior production country, but omitted production country remains null instead of being fabricated. Watch, interior-clock, and accessory detail groups may be omitted or partial for every status; profile services upsert submitted edit groups, preserve explicit boolean `false`, and treat explicit nulls as clears. RU name is the required/default translation; EN/KZ names and descriptions fall back to RU, and absent descriptions persist as empty long-text rows. `ProductDescriptionMapper` writes long copy only to `content_md`; it never copies descriptions into the 255-character `title` column. | `ProductService`, `ProductBulkCreationService`, `SupportedBrandCountry`, `SupportedCatalogDomainService`, `MarketplaceUrlPolicy`, `SKUService`, `ProductDescriptionMapper`, `WatchDetailServiceImpl`, `InteriorClockDetailServiceImpl`, `AccessoryDetailService` |
| Kaspi product preview importer | `services/marketplace/kaspi/*`, `dtos/product/KaspiProductImport*.java`, `exceptions/ProductImportException.java` | Fetches one allow-listed HTTPS Kaspi product page through a fair bounded concurrency gate with explicit redirect revalidation, DNS/private-network rejection, direct no-proxy HTTP, one total deadline, content-type/encoding checks, and a cancelling 2 MiB streaming cap. It parses JSON-LD first with constrained embedded/semantic DOM fallbacks, ignores aggregate specification rows, rejects ambiguous numeric/range/fraction/length input instead of truncating or guessing, resolves fields through category-profile-specific exact/normalized/alias dictionaries, rejects conflicting duplicate targets and interior brand-country mismatches, distinguishes unresolved from unmapped characteristics, and returns a safe preview without calling product persistence. | `KaspiUrlGuard`, `SecureKaspiPageFetcher`, `KaspiProductParser`, `KaspiCharacteristicMapper`, `KaspiProductImportService`, `KaspiProductImportResponse` |
...
| CRM product create | `POST /api/crm/products?lang=` | Essential fields: category, nonblank RU/default `productName.name_ru` (max 255), model (max 255), non-negative integer price/stock, one supported brand, status/type (CRM defaults new items to `DRAFT`). Optional: EN/KZ names, every description (max 10,000), collection, photos, marketplace URLs, and partial category-matching `watchDetails`, `interiorClockDetails`, or `accessoryDetails`. Accessory details accept nullable clasp type, case/insert material ids, `hasInsert`, color id, and free-text length. A supplied interior `productionCountryId` must equal the selected brand's canonical country; omission persists null. Missing EN/KZ names/descriptions fall back to RU. | `ProductResponse`; `UNSUPPORTED_BRAND` or `BRAND_COUNTRY_MISMATCH` for catalog-domain violations; audit event `PRODUCT_CREATED` | CRM add product form after the category-first step; all technical fields are optional |
| CRM product bulk create | `POST /api/crm/products/bulk?lang=` | `ProductBulkCreateRequest` with `categoryId`, `common` defaults for supported brand/status/type/RU-EN-KZ descriptions/specs/links, and rows with names/model/price/stock plus optional per-row brand/collection/status/type/RU-EN-KZ descriptions/watch/interior/accessory overrides/links. Row values win over common defaults; every effective row brand/country pair is validated independently, blank ids are omitted, and explicit accessory `false` remains false. | `ProductBulkCreateResponse` with created/failed counts, created ids/SKUs, and row-level errors including catalog-domain rejection text; audit event `PRODUCT_BULK_CREATED` | CRM `/products/bulk` form with profile-specific optional common and row controls |
| CRM product update | `PUT /api/crm/products/{id}?lang=` | `ProductUpdateRequest` with editable core fields, one supported brand, locked/same-profile category, description `desc_ru/desc_en/desc_kz`, optional watch/interior/accessory details according to selected category profile, stock, and optional marketplace URLs. Backend rejects unsupported brands, mismatched submitted countries, detail blocks outside the target profile, and cross-profile category edits. Submitted detail groups are upserted for legacy rows; explicit null detail members clear saved values. | `ProductResponse`; audit event `PRODUCT_EDITED` | CRM edit product form with optional localized technical fields |
...
| CRM references | `GET /api/crm/references?lang=` | Bearer access token | Categories, collections, mechanisms, genders, materials, glass types, stone inlays, interior colors/styles/mechanisms/power types, exactly six supported brands, and exactly five supported countries. Each brand option includes its canonical `countryId`, country code, and localized country name; collections include `brandId`. | CRM product selects plus read-only create/edit/bulk country derivation; payloads submit stable canonical IDs |
...
* Frontend tests: Public Vitest covers recommendation behavior, local product summaries, 429 parsing, CMS revalidation, canonical routes, metadata, carousel behavior, and product Markdown rendering/sanitization/plain-line compatibility; the current storefront suite runs 36 files / 150 tests. CRM Vitest covers auth/session behavior, CMS feedback, canonical brand-country derivation, Markdown sanitization, the Kaspi API request contract, create-only dialog focus/scroll/close states, preview readability, repeated import prevention, failed-import isolation, cancel/apply behavior, profile-safe RHF merging, manual edits after import, and the unchanged normal submit path; the current CRM suite runs 15 files / 73 tests. Both apps require lint, typecheck, finite tests, npm audits, and production builds. Dependency changes additionally require clean npm 10.9.8 Linux `npm ci` through the pinned Docker dependency stages.
* Backend tests: The Java 17 suite currently runs 59 suites / 244 tests (one skipped environment-gated rehearsal) and covers the Step 4/5/6 baseline plus Step 7 product activation/final-image rules, collision-free media/CMS reordering, stable constraint error codes, fresh-chain constraints, direct invalid SQL, concurrent stock/media/CMS writes, ADMIN bootstrap validation/creation/idempotency/collision/log-safety behavior, authoritative supported-domain validation, individual/bulk mismatches, interior-country derivation/realignment, and Kaspi URL/redirect SSRF defense, total-deadline/body/bulkhead fetch bounds, adversarial parsing and mapping, non-persistence, layered rate limits, migration boundary checks, and fail-closed rollback contracts.
...
2026-09-03 - Hardened Kaspi preview parsing/mapping/fetch concurrency and total deadlines, made importer migration rollback and packaged-change verification fail closed, fixed viewport-bounded CRM modal scrolling/focus, expanded adversarial coverage, and verified the rebuilt local Compose import/create/edit/archive round-trip.
