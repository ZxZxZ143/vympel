...
* Monorepo/workspace tool: None; repository contains two sibling apps.
...
* Docker/DevOps: Backend includes an opt-in multi-stage non-root image plus Compose services for persistent local Redis and MinIO; the backend Compose profile is explicitly local-only. No CI workflow was discovered.
