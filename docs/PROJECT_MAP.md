...
| Variable | Required | Description |
| -------- | -------- | ----------- |
| `NEXT_PUBLIC_CRM_API_BASE` | yes for CRM API calls | Protected backend CRM API base URL exposed to the CRM browser app. It is required in every mode; local development supplies it through ignored `vympel_crm/.env` rather than a compiled fallback. |
...
The 2026-08-19 SHA-only preview publication contract compiles the storefront for `https://preview.vympel.kz`, public API/media for `https://api.vympel.kz`, staging environment, exact commit release metadata, disabled telemetry, and explicit `SITE_INDEXING_ENABLED=false`. CRM keeps `NEXT_PUBLIC_CRM_API_BASE=https://api.34.18.200.58.sslip.io/api/crm`; no `crm.vympel.kz` or public `api.vympel.kz/api/crm` contract is introduced. `deployment/scripts/verify-preview-indexing.sh` runs against the exact published amd64 and ARM64 storefront images and requires global noindex, no canonical/alternate/social discovery metadata, no advertised sitemap, and no sitemap locations. Publication uses only the full 40-character SHA and creates no RC or mutable alias. The checked-in GCP env remains historical sslip deployment state until a separately authorized VM/Nginx/DNS/TLS change.
