...
* **How to capture fresh backend release evidence:** If `gradlew test` reports `UP-TO-DATE`, rerun the finite suite with `gradlew test --rerun-tasks`; a cached task result is useful locally but is not a fresh pre-release execution record.
