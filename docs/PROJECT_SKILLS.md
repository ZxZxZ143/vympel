...
* **How:** Keep the endpoint authenticated, rate-limited, non-transactional, and preview-only. Accept only the exact HTTPS marketplace hosts and product path, reject credentials/non-default ports/private or special resolved addresses, disable ambient proxies, revalidate every redirect, bound redirects/connect/request time/content type/body size, and return safe domain error codes. Parse structured product data first, then constrained embedded/semantic fallbacks. Map only through the selected category profile and existing reference dictionaries using exact, normalized, then explicit-alias resolution; report unknown labels as unmapped and known labels with unknown values as unresolved. Never call product repositories or create dictionary rows from scraped text.
* **Why:** Remote HTML is untrusted and unstable. A bounded preview keeps SSRF, parser drift, guessed references, and accidental persistence out of the normal product-create contract.
...
* **How:** Require category selection before offering import; send the category with the URL; render mapped, unmapped, unresolved, and warning groups in an accessible dialog; and on Apply merge only non-null preview values into the current RHF state. Preserve category/profile, keep current values where import data is absent or unresolved, preserve meaningful `false` and `0`, and let later manual edits win. The regular product-create request remains the sole persistence path.
...
* **How to verify Kaspi import:** Run backend `KaspiUrlGuardTest`, `KaspiProductParserTest`, `KaspiCharacteristicMapperTest`, `KaspiProductImportServiceTest`, `MarketplaceUrlPolicyTest`, `RateLimitFilterTest`, and the stainless migration contract test; then run the full backend suite and `bootJar`. Run CRM `npm test`, `npm run lint`, `npm run typecheck`, and `npm run build`. Finally build `docker compose up -d --build --wait`, verify Liquibase rows/material translations/URL constraint directly, call the authenticated preview with a real Kaspi product URL, assert no product was persisted, and check backend readiness plus CRM/storefront pages.
...
2026-09-01 - Recorded the secure preview-only Kaspi fetch/parse/map pattern, profile-safe RHF apply semantics, Spring Boot 4/Jackson 3 constructor and namespace gotchas, deterministic importer verification, npm 10 lockfile proof, and Docker Desktop stale-socket recovery used for the healthy local stand.
