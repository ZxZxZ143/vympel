...
* **How:** Route all four surfaces through `CatalogOverlayProvider`; store only a safe trigger element and keep one `activeOverlay`. Never register the desktop catalog search input as a restore target because its `onFocus` opens search. Keep the compact search trigger mounted (visually hidden while open) so the same connected DOM node can receive focus after the 220ms exit transition. Below 1024px render modal surfaces through `CatalogMobileSheet`/Radix Dialog, let Radix own focus trapping, Escape, portal layering, and body scroll lock, and hide `MobileBottomNavigation` until the exit completes. Mobile filter content has one internal vertical scroller with safe-area Apply/Reset actions; mobile categories use the single `100dvh` tree shared by toolbar and bottom-nav triggers. From 1024-1439px reuse the compact triggers but open the existing toolbar-attached category/filter/sort panels instead of phone-sized sheets.
...
### Deterministic catalog search close and toolbar height

* **When to use:** When editing catalog search open/close behavior, focus restoration, or hero-toolbar control sizing.
* **How:** Derive catalog search visibility only from `CatalogOverlayProvider`; use explicit `open` and `close` reducer actions instead of toggles or a second local open flag. A close for an inactive overlay is a no-op. Keep the desktop auto-open input out of focus restoration, retain the compact trigger node while the overlay is open, and use the shared `h-12` toolbar height class for search, category, filter, and sort triggers at every breakpoint.
* **Why:** Restoring focus to an `onFocus` opener recreated the overlay after its 220ms close transition, while separate height rules made the search field taller than adjacent controls.
...
### Public product marketplace action removal

* **When to use:** When a marketplace field must remain in product data/CRM but its purchase CTA must disappear from the public product detail page.
* **How:** Remove the anchors and their wrapper directly from `ProductSummary`; do not hide them with CSS or change `kaspiUrl`/`wildberriesUrl`, API types, backend policy, CRM, Home/About marketplace cards, or the non-interactive Kaspi/Halyk payment tiles. Put `mt-product-summary-actions-gap` on the next existing action container in both available and unavailable states; `globals.css` owns the exact `--spacing-product-summary-actions-gap: 28px` value.
* **Why:** Display policy can change without breaking marketplace data ownership, while one named spacing owner prevents an empty container, doubled margin, keyboard ghost actions, or a collapsed product summary.
...
### Localized route metadata from real content

* **When to use:** Adding or refining title/description for public Home, Catalog/Category, Product, Brands/Brand, About, Delivery, Payment, or Guarantee routes.
* **How:** Keep route owners on `publicSeoMetadata` and put locale-specific copy/composition in `seoContent.ts`. Static routes use explicit RU/KZ/EN content; category metadata uses the localized API category name; brand metadata uses the configured localized visible description; product metadata reuses `loadProductPageData` through `React.cache`, composes missing brand/model tokens without duplication, and runs real localized description text through `plainSeoText` plus word-boundary shortening. Preserve fail-closed indexing, 404/transient separation, canonical/hreflang, and matching OG/Twitter fields.
* **Why:** Search metadata must describe the current route and real resource in initial HTML without introducing duplicate API requests, fake characteristics, cross-locale leakage, or a parallel SEO framework.
...
* **How:** Conditionally mount disclosure content, close on Escape, and restore trigger focus. Use Radix Dialog for modal surfaces so focus trap, scroll lock, outside content suppression, and close focus are owned centrally. Mark only offscreen Embla slides `aria-hidden`/`inert`, subscribe to `slidesInView`, and fall back to `selectedScrollSnap()` when the observer is temporarily empty. The Home hero is the only autoplay exception: it has no visual Play/Pause control, starts automatically only without reduced motion, pauses on whole-carousel mouse hover and keyboard-origin focus, then resumes on leave/blur and drag completion. Keep every other carousel manual.
...
### Normalize Wildberries characteristics before shared dictionary resolution

* **When to use:** Mapping Wildberries detail `options` into category-owned CRM watch fields.
* **How:** Keep the source rows intact for preview, but pass WB-only normalized label/value copies through `WildberriesCharacteristicMapper` before reusing `KaspiCharacteristicMapper`. Map only explicit aliases; accept case size only as a whole millimetre value; normalize unambiguous WR30/50/100/200 forms; split known feature aliases across commas, semicolons, bars, bullets, slashes, and newlines; strip repeated `Комплектация:` prefixes; and never fan generic `Цвет` into strap/dial fields. Report every original row exactly once as mapped, unmapped, or unresolved, including equivalent duplicates and every side of a conflict.
* **Why:** Marketplace vocabularies overlap but are not interchangeable. A source adapter preserves shared exact/alias dictionary matching without hiding source rows, guessing field meaning, or changing Kaspi behavior.

### Keep Wildberries descriptions out of the product form

* **When to use:** Parsing, previewing, displaying, or applying a Wildberries product import.
* **How:** Ignore `details.description`, do not synthesize description text from characteristics, return `descriptionRu=null`, filter any defensive mapped description on the backend and in the WB preview UI, and call the shared apply helper with description assignment disabled. Preserve both non-empty and empty manual RU descriptions. Kaspi continues to import its description through the original path.
* **Why:** Wildberries descriptions frequently concatenate marketing copy and technical characteristics; importing them overwrites curated product content and hides characteristics that need explicit mapping outcomes.
...
* **How:** Keep source URL validation, API call, labels, safe errors, and warnings source-specific while reusing the accessible focus-trapped, viewport-bounded dialog shell and RHF apply function. Apply only present/resolved values, preserve the selected category/product type and manual values with no resolution, clear brand-dependent stale fields through the existing path, and leave the ordinary Save action as the sole persistence step. Wildberries must additionally preserve manual `descriptionRu` unconditionally and use the exact localized mapped/unmapped/unresolved/warnings section labels.
...
* **How:** Preserve the Figma/task structure: hero with rounded controls, localized search-only toolbar, localized breadcrumbs, 181x181 desktop thumbnails, a 475x502 desktop clickable main image with localized zoom dialog, right summary block, five localized info tabs including reviews, description/specs two-column layout, related products, and localized dark contact CTA. The public product summary no longer contains the screenshot's Kaspi/Wildberries pill purchase buttons; price-to-cart/request spacing is exactly the shared 28px token, while static Kaspi/Halyk payment badges remain.
...
### Letting transient carousel interaction stop autoplay permanently

* **What happened:** Home hero autoplay behavior was split between a manual Play/Pause control, root handlers, and Embla interaction state; drag, resize/reInit, or pointer-click focus could leave autoplay stopped with no reliable recovery.
* **Root cause:** DOM event state was treated as permanent intent, pointer-origin focus was not distinguished from keyboard focus, and reInit could destroy an active drag's pending pointer-up listener.
* **Fix:** Remove the visual control and centralize lifecycle authority in `useCarouselAutoplayLifecycle`: mouse hover and keyboard-origin descendant focus pause, pointer-origin focus does not outlive the pointer interaction, the shared Carousel's non-tabbable programmatic root-focus fallback is ignored, pointer-up resumes after drag, reInit clears stale drag state, visibility restoration reasserts the current policy, and cleanup owns every Embla/media/document listener plus pending animation frames and `plugin.stop()`.
* **How to avoid:** Test repeated hover cycles, pointer-click -> focus -> mouseleave, pointer-down without focus -> later keyboard focus, keyboard focus retention, touch/mouse drag, reInit during an active drag, reduced-motion changes, visibility restoration, multiple instances, and unmount cleanup. Keep modality tracking on bubbling `onKeyDown`; a consumer `onKeyDownCapture` would overwrite the shared Carousel's ArrowLeft/ArrowRight handler because its props are spread last.
...
* **How to verify carousel autoplay policy:** Search the complete storefront carousel inventory and assert only `HomePage/bannerCarousel/autoplay.ts` imports Embla Autoplay. Browser-check automatic slide advance, no Play/Pause control, whole-carousel hover pause for longer than the 5-second delay, mouseleave resume, arrow/dot/manual drag followed by resumed autoplay, touch drag recovery, keyboard-focus pause, pointer-click focus followed by leave, and reduced-motion suppression. About, Brands, product rails/recommendations, and gallery must remain manual. Run the lifecycle unit suite to cover reInit-during-drag, visibility restoration, instance isolation, listener removal, animation-frame cancellation, and final plugin stop.
...
* **How to verify Wildberries import:** Run backend `WildberriesUrlGuardTest`, `SecureWildberriesProductFetcherTest`, `WildberriesProductParserTest`, `WildberriesCharacteristicMapperTest`, `WildberriesProductImportServiceTest`, `CrmProductControllerWildberriesTest`, `CrmProductControllerKaspiTest`, `WildberriesProductSourceLinksMigrationContractTest`, `KaspiCharacteristicMapperTest`, and `MarketplaceUrlPolicyTest`, then the complete Java 17 suite and `bootJar`. Run every CRM test containing Wildberries coverage, then `npm test -- --run`, `npm run lint`, `npm run typecheck`, and `npm run build`. Keep automated payloads in sanitized fixtures; any real-card probe must be a temporary, read-only smoke check and must not become an always-online test. In the real create form, confirm both marketplace actions remain available after category selection; Wildberries preview must show the exact RU main/mapped/unmapped/unresolved/warnings sections, every source characteristic must have exactly one outcome, and Apply must retain category/product type plus both empty and non-empty manual RU descriptions while populating only resolved non-description fields. Invalid centimetre case size must stay blank, generic color must not populate two targets, conflicting water values must remain unresolved, and no request may create a product or dictionary row before the normal Save action.
...
* Product page summary sizing: Title `text-product-title`/`leading-product-title`, gender/category `text-product-meta text-text-product-muted`, price `Text size="h5"`, price-to-cart/request gap `--spacing-product-summary-actions-gap: 28px`, cart button `311x50 bg-button-bg-product` with `Text size="bodyMd"`, favorite button `50x50`, and non-interactive payment tiles `74x74`.
* Product payment badges: Reuse `/kaspiIcon.png` and `/halykIcon.png` as complete branded 148x148 tiles. Keep the existing `size-14 sm:size-[74px]` wrappers and `gap-3 sm:gap-5`, render with `object-contain`, use localized image alternatives, and keep wrappers as static `div` elements without links, buttons, handlers, pointer styling, or focusability.
...
| `org.apache.tomcat.embed` | 11.0.25 (managed-property override) | backend | Spring Boot 4.0.8 otherwise resolves 11.0.24, which the release image scan rejects for the security-constraint fixes published in 11.0.25. Override only `tomcat.version`, verify all Tomcat embed modules remain aligned through `dependencyInsight`, rerun the full backend suite/JAR, and require the image Trivy gate before publication. |
...
* **Why:** RC.8 through RC.11 demonstrated that workflow success and artifact generation can be corroborated without trusting either record alone, while preserving the immutability guarantee of earlier candidates.
...
2026-09-11 - Documented the Wildberries-only characteristic adapter, exact outcome accounting, strict description/model boundaries, normalization rules, sanitized fixture strategy, and expanded backend/CRM verification matrix while preserving Kaspi behavior.
